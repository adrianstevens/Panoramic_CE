...
BOOL CDlgGetFile::OnInitDialog(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	if(m_bOpenFile)
		SetWindowText(m_hWnd, _T("Open File"));
	else
		SetWindowText(m_hWnd, _T("Save File"));
...
void CDlgGetFile::OnMenuOpen()
{
	...
	{
		MessageBox(m_hWnd, _T("File does not exist"), _T("Error"), MB_OK | MB_ICONEXCLAMATION);
		return;
	}
...
{
    ::DrawText(hDC, _T("Name:"), m_rcName, DT_LEFT|DT_VCENTER, m_hFontText, RGB(0,0,0));
    ::DrawText(hDC, _T("Type:"), m_rcType, DT_LEFT|DT_VCENTER, m_hFontText, RGB(0,0,0));
...
void CDlgGetFile::SaveFile(TCHAR* szFileName)
{
	...
	else if(dwAttributes != ((DWORD)-1))
	{
		...
		if(IDYES != MessageBox(m_hWnd, _T("File already exists.  Would you like to overwrite?"), _T("Warning"), MB_YESNO | MB_ICONQUESTION))
			return;
	}
...
BOOL CCtrlBrowse::GetCurrentFolderName(TCHAR* szFolderName)
{
	...
	if(IsRootPath())
	{
		m_oStr->StringCopy(szFolderName, _T("My Device"));
		return TRUE;
	}
