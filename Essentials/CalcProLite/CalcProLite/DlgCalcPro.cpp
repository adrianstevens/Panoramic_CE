...
BOOL CDlgCalcPro::OnPaint(HWND hWnd, HDC hDC, RECT& rcClient)
{
	...
	//	rcTemp.left = 0;
	//	rcTemp.right = GetSystemMetrics(SM_CXSCREEN);
	//	rcTemp.top = 0;
	//	rcTemp.bottom = GetWindowHeight(hWnd);
		m_gdiMem.Create(hDC, rcTemp, FALSE, FALSE);
	}
	if(m_oInterface)
		m_oInterface->Draw(m_gdiMem.GetDC(), rcClip);
