...
,m_oInterface(FALSE)
{
	...
	if(S_OK == GetKey(REG_KEY_FIN, REG_PS, dwVal))
...
CDlgCalcPro::~CDlgCalcPro(void)
{
	...
	SetKey(REG_KEY_FIN, REG_PS, dwVal);
...
//        HWND hWndSip = FindWindow(_T("MS_SIPBUTTON"), NULL);
  //      ShowWindow(hWndSip, SW_SHOW);
    ...
    m_oKey.Init(_T("SOFTWARE\\Panoramic\\PanoFinCalc"), _T("SOFTWARE\\Pano\\PFC2"), 0x0, 0xCDF4312A, 0x541AD34B, 4, TRUE);
    m_oKey.CheckSetKey();
    m_oKey.CheckDemo();
