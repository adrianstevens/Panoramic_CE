#include "IssFinInterface.h"
#include "IssRegistry.h"
#include "DlgFinOptions.h"
#include "DlgAbout.h"
#include "Resource.h"
#include "IssGDIFx.h"
...
void CIssCalcInterface::LoadRegistry()
{
	...
	//regional settings
	if(S_OK == GetKey(REG_KEY_FIN, _T("RegionalSettings"), dwTemp))
		m_bUseRegionalSettings = (BOOL)dwTemp;
	else 
		m_bUseRegionalSettings = FALSE;

	if(S_OK == GetKey(REG_KEY_FIN, _T("PlaySounds"), dwTemp))
		m_bPlaySounds = (BOOL)dwTemp;
	else 
		m_bPlaySounds = TRUE;

	//use tape
	if(S_OK == GetKey(REG_KEY_FIN, _T("UseTape"), dwTemp))
		m_oCalc->SetUseTape((BOOL)dwTemp);
	else 
		m_oCalc->SetUseTape(FALSE);

	//animate 2ndhyp
	if(S_OK == GetKey(REG_KEY_FIN, _T("Ani2ndHyp"), dwTemp))
		m_bAnimate2ndHyp = (BOOL)dwTemp;
	else 
		m_bAnimate2ndHyp = (FALSE);

	//set the current skin
	if(S_OK == GetKey(REG_KEY_FIN, _T("Skin"), dwTemp))
		m_oBtnManger.SetSkinType((EnumCalcProSkins)dwTemp, NULL);
	else
		m_oBtnManger.SetSkinType(SKIN_Blue, NULL);

	//calculator type
	if(S_OK == GetKey(REG_KEY_FIN, _T("CalcType"), dwTemp))
		m_oBtnManger.SetCalcType((EnumCalcType)dwTemp, m_hInst);
	else
		m_oBtnManger.SetCalcType(CALC_Financial, m_hInst);

	//set the calc mode
	if(S_OK == GetKey(REG_KEY_FIN, _T("CalcMode"), dwTemp))
		m_eCalcMode = (CalcOperationType)dwTemp;
    UpdateCalcMode(m_eCalcMode);

	//set the base bits
	if(S_OK == GetKey(REG_KEY_FIN, _T("BaseBits"), dwTemp))
		m_oCalc->SetBaseBits((__int8)dwTemp);
	else
		m_oCalc->SetBaseBits(32);

	//set the notation
	if(S_OK == GetKey(REG_KEY_FIN, _T("Notation"), dwTemp))
		m_oCalc->SetCalcDisplay((DisplayType)dwTemp);
	else
		m_oCalc->SetCalcDisplay(DISPLAY_Float);

	//set the RPN Stack Size
	if(S_OK == GetKey(REG_KEY_FIN, _T("RPNStack"), dwTemp))
		m_oCalc->SetRPNStackSize((__int8)dwTemp);
	else
		m_oCalc->SetRPNStackSize(4);

	if(S_OK == GetKey(REG_KEY_FIN, _T("FixedDigits"), dwTemp))
		m_oCalc->SetFixedDecDigits((int)dwTemp);


    if(S_OK == GetKey(REG_KEY_FIN, _T("TVMEndPay"), dwTemp))
        g_bTVMEndPay = ((int)dwTemp);
    else
        g_bTVMEndPay = FALSE;

    if(S_OK == GetKey(REG_KEY_FIN, _T("MortPaymentSched"), dwTemp))
        g_ePaymentSched = (PAYMENT_SCHEDULE)dwTemp;
    else
        g_ePaymentSched = PAYMENT_MONTHLY;

    if(S_OK == GetKey(REG_KEY_FIN, _T("GraphFast"), dwTemp))
        g_bGraphFast = ((int)dwTemp);

    if(S_OK == GetKey(REG_KEY_FIN, _T("GraphPoints"), dwTemp))
        g_bGraphPoints = ((int)dwTemp);

    if(S_OK == GetKey(REG_KEY_FIN, _T("DRG"), dwTemp))
    ...
    for(int i = 0; i < NUMBER_OF_MemoryBanks; i++)
    {
        ...
        if(S_OK == GetKey(REG_KEY_FIN, szMemLabel, szTemp, STRING_MAX))
...
void CIssCalcInterface::SaveRegistry()
{
    ...
    SetKey(REG_KEY_FIN, _T("UseTape"), dwValue);

    for(int i = 0; i < NUMBER_OF_MemoryBanks; i++)
    {
        m_oStr->Format(szMemLabel, _T("Memory%i"), i);
        m_oCalc->GetMemory(szTemp, i);
        SetKey(REG_KEY_FIN, szMemLabel, szTemp, m_oStr->GetLength(szTemp));
    }

	dwValue = (DWORD)m_oBtnManger.GetCalcType();
	SetKey(REG_KEY_FIN, _T("CalcType"), dwValue);

	dwValue = (DWORD)m_eCalcMode;
	SetKey(REG_KEY_FIN, _T("CalcMode"), dwValue);

	dwValue = (DWORD)m_oCalc->GetRPNStackSize();
	SetKey(REG_KEY_FIN, _T("RPNStack"), dwValue);

	dwValue = (DWORD)m_oCalc->GetCalcDisplay();
	SetKey(REG_KEY_FIN, _T("Notation"), dwValue);

	dwValue = (DWORD)m_oCalc->GetBaseBits();
	SetKey(REG_KEY_FIN, _T("BaseBits"), dwValue);

	dwValue = (DWORD)m_bUseRegionalSettings;
	SetKey(REG_KEY_FIN, _T("RegionalSettings"), dwValue);

    dwValue = (DWORD)m_oBtnManger.GetUseAdvMem();
    SetKey(REG_KEY_FIN, _T("AdvMem"), dwValue);

	dwValue = (DWORD)m_bPlaySounds;
	SetKey(REG_KEY_FIN, _T("PlaySounds"), dwValue);

	dwValue = (DWORD)m_oBtnManger.GetSkinType();
	SetKey(REG_KEY_FIN, _T("Skin"), dwValue);

	dwValue = (DWORD)m_bAnimate2ndHyp;
	SetKey(REG_KEY_FIN, _T("Ani2ndHyp"), dwValue);

	dwValue = (DWORD)m_oCalc->GetFixedDecDigits();
	SetKey(REG_KEY_FIN, _T("FixedDigits"), dwValue);


    dwValue = (DWORD)g_bTVMEndPay;
    SetKey(REG_KEY_FIN, _T("TVMEndPay"), dwValue);

    dwValue = (DWORD)g_ePaymentSched;
    SetKey(REG_KEY_FIN, _T("MortPaymentSched"), dwValue);

    dwValue = (DWORD)g_bGraphFast;
    SetKey(REG_KEY_FIN, _T("GraphFast"), dwValue);

    dwValue = (DWORD)g_bGraphPoints;
    SetKey(REG_KEY_FIN, _T("GraphPoints"), dwValue);

    dwValue = (DWORD)m_oCalc->GetDRGState();
    SetKey(REG_KEY_FIN, _T("DRG"), dwValue);
...
BOOL CIssCalcInterface::OnMenuLeft()
{
    ...
    m_oStr->StringCopy(szText, ID(IDS_MENU_Options), STRING_MAX, m_hInst);
...
BOOL CIssCalcInterface::OnMenuRight()
{
    ...
    int iChecked = 0;
    if(m_oBtnManger.GetCalcType() == CALC_Financial)
        iChecked = 0;
...
void CIssCalcInterface::LaunchHelp()
{
	...
	{
	case LANG_FRENCH:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_fr.htm"));
		break;
	case LANG_GERMAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_de.htm"));
		break;
	case LANG_SPANISH:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_es.htm"));
		break;
	case LANG_JAPANESE:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_jp.htm"));
		break;
	case LANG_DUTCH:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_du.htm"));
		break;
	case LANG_KOREAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_ko.htm"));
		break;
	case LANG_ITALIAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_it.htm"));
		break;
	case LANG_PORTUGUESE:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_pt.htm"));
		break;
	case LANG_RUSSIAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial_ru.htm"));
		break;
	default:
		m_oStr->StringCopy(szHelpFile, _T("PanoFinancial.htm"));
		break;
	}
...
BOOL CIssCalcInterface::NextCalc(BOOL bForward)
{
    ...
    if(eNextCalc == CALC_Financial)
        eNextCalc = CALC_Statistics;
    else
        eNextCalc = CALC_Financial;
