// Financial.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "Financial.h"
#include "DlgFinancial.h"
#include "IssCommon.h"
#include "resource.h"
#include "IssKey.h"
#include "IssString.h"
...
#define CLASS_Name  _T("ClassFinancial")
#define REG_KEY_FIN _T("SOFTWARE\\Pano\\Financial") 
                   ...
                   int       nCmdShow)
{
	...
	if(!dlgCalcPro)
		return FALSE;
	if(dlgCalcPro->Create(_T("Financial"), NULL, hInstance, CLASS_Name))
	{
    ...
    if(!key.m_bGood)
    {
        ...
        oStr->StringCopy(szText, IDS_MSG_DemoOver, STRING_MAX, hInstance);

        int iFind = oStr->Find(szText, _T("Pro"));
        if(iFind != -1)
        {
            oStr->Delete(iFind - 1, 4, szText);//get the space
            oStr->Insert(szText, _T("Financial "), iFind - 5);
        }

        MessageBox(GetDesktopWindow(),szText, _T("Panoramic Financial Calc"), MB_OK | MB_ICONASTERISK);
        return FALSE;*/
