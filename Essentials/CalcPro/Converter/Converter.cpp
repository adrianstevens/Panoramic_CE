...
#define CLASS_Name  _T("ClassConverter")
#define REG_KEY_CON _T("SOFTWARE\\Pano\\Converter") 
                   ...
                   int       nCmdShow)
{
	...
	if(!dlgCalcPro)
		return FALSE;
	if(dlgCalcPro->Create(_T("Converter"), NULL, hInstance, CLASS_Name))
	{
    ...
    key.Init(_T("SOFTWARE\\Panoramic\\PanoConverter"), _T("SOFTWARE\\Pano\\PCC2"), 0x0, 0x2CB2BFCC, 0x86345EEF, 4, TRUE);
    key.CheckSetKey();
    key.CheckDemo();
    ...
    if(!key.m_bGood)
    {
        ...
        int iFind = oStr->Find(szText, _T("Calc"));
        if(iFind != -1)
        {
            oStr->Delete(iFind, 4, szText);//get the space
            oStr->Insert(szText, _T("Converter"), iFind);
        }

        MessageBox(GetDesktopWindow(),szText, _T("Panoramic Converter Pro"), MB_OK | MB_ICONASTERISK);
        return FALSE;*/
