#include "IssConInterface.h"
#include "IssRegistry.h"
#include "DlgConOptions.h"
#include "DlgConAbout.h"
#include "Resource.h"
#include "IssGDIFx.h"
...
void CIssCalcInterface::LoadRegistry()
{
	...
	//regional settings
	if(S_OK == GetKey(REG_KEY_CON, _T("RegionalSettings"), dwTemp))
		m_bUseRegionalSettings = (BOOL)dwTemp;
	else 
		m_bUseRegionalSettings = FALSE;

	if(S_OK == GetKey(REG_KEY_CON, _T("PlaySounds"), dwTemp))
		m_bPlaySounds = (BOOL)dwTemp;
	else 
		m_bPlaySounds = TRUE;

	//use tape
	if(S_OK == GetKey(REG_KEY_CON, _T("UseTape"), dwTemp))
		m_oCalc->SetUseTape((BOOL)dwTemp);
	else 
		m_oCalc->SetUseTape(FALSE);

	//animate 2ndhyp
	if(S_OK == GetKey(REG_KEY_CON, _T("Ani2ndHyp"), dwTemp))
		m_bAnimate2ndHyp = (BOOL)dwTemp;
	else 
		m_bAnimate2ndHyp = (FALSE);

	//set the current skin
	if(S_OK == GetKey(REG_KEY_CON, _T("Skin"), dwTemp))
		m_oBtnManger.SetSkinType((EnumCalcProSkins)dwTemp, NULL);
	else
		m_oBtnManger.SetSkinType(SKIN_Blue, NULL);

	//calculator type
	if(S_OK == GetKey(REG_KEY_CON, _T("CalcType"), dwTemp))
		m_oBtnManger.SetCalcType((EnumCalcType)dwTemp, m_hInst);
	else
		m_oBtnManger.SetCalcType(CALC_Currency, m_hInst);

	//set the calc mode
	if(S_OK == GetKey(REG_KEY_CON, _T("CalcMode"), dwTemp))
		m_eCalcMode = (CalcOperationType)dwTemp;
    UpdateCalcMode(m_eCalcMode);

	//set the base bits
	if(S_OK == GetKey(REG_KEY_CON, _T("BaseBits"), dwTemp))
		m_oCalc->SetBaseBits((__int8)dwTemp);
	else
		m_oCalc->SetBaseBits(32);

	//set the notation
	if(S_OK == GetKey(REG_KEY_CON, _T("Notation"), dwTemp))
		m_oCalc->SetCalcDisplay((DisplayType)dwTemp);
	else
		m_oCalc->SetCalcDisplay(DISPLAY_Float);

	//set the RPN Stack Size
	if(S_OK == GetKey(REG_KEY_CON, _T("RPNStack"), dwTemp))
		m_oCalc->SetRPNStackSize((__int8)dwTemp);
	else
		m_oCalc->SetRPNStackSize(4);

	if(S_OK == GetKey(REG_KEY_CON, _T("FixedDigits"), dwTemp))
		m_oCalc->SetFixedDecDigits((int)dwTemp);


    if(S_OK == GetKey(REG_KEY_CON, _T("TVMEndPay"), dwTemp))
        g_bTVMEndPay = ((int)dwTemp);
    else
        g_bTVMEndPay = FALSE;

    if(S_OK == GetKey(REG_KEY_CON, _T("GraphFast"), dwTemp))
        g_bGraphFast = ((int)dwTemp);

    if(S_OK == GetKey(REG_KEY_CON, _T("GraphPoints"), dwTemp))
        g_bGraphPoints = ((int)dwTemp);

    if(S_OK == GetKey(REG_KEY_CON, _T("DRG"), dwTemp))
    ...
    for(int i = 0; i < NUMBER_OF_MemoryBanks; i++)
    {
        ...
        if(S_OK == GetKey(REG_KEY_CON, szMemLabel, szTemp, STRING_MAX))
...
void CIssCalcInterface::SaveRegistry()
{
    ...
    SetKey(REG_KEY_CON, _T("UseTape"), dwValue);

    for(int i = 0; i < NUMBER_OF_MemoryBanks; i++)
    {
        m_oStr->Format(szMemLabel, _T("Memory%i"), i);
        m_oCalc->GetMemory(szTemp, i);
        SetKey(REG_KEY_CON, szMemLabel, szTemp, m_oStr->GetLength(szTemp));
    }

	dwValue = (DWORD)m_oBtnManger.GetCalcType();
	SetKey(REG_KEY_CON, _T("CalcType"), dwValue);

	dwValue = (DWORD)m_eCalcMode;
	SetKey(REG_KEY_CON, _T("CalcMode"), dwValue);

	dwValue = (DWORD)m_oCalc->GetRPNStackSize();
	SetKey(REG_KEY_CON, _T("RPNStack"), dwValue);

	dwValue = (DWORD)m_oCalc->GetCalcDisplay();
	SetKey(REG_KEY_CON, _T("Notation"), dwValue);

	dwValue = (DWORD)m_oCalc->GetBaseBits();
	SetKey(REG_KEY_CON, _T("BaseBits"), dwValue);

	dwValue = (DWORD)m_bUseRegionalSettings;
	SetKey(REG_KEY_CON, _T("RegionalSettings"), dwValue);

    dwValue = (DWORD)m_oBtnManger.GetUseAdvMem();
    SetKey(REG_KEY_CON, _T("AdvMem"), dwValue);

	dwValue = (DWORD)m_bPlaySounds;
	SetKey(REG_KEY_CON, _T("PlaySounds"), dwValue);

	dwValue = (DWORD)m_oBtnManger.GetSkinType();
	SetKey(REG_KEY_CON, _T("Skin"), dwValue);

	dwValue = (DWORD)m_bAnimate2ndHyp;
	SetKey(REG_KEY_CON, _T("Ani2ndHyp"), dwValue);

	dwValue = (DWORD)m_oCalc->GetFixedDecDigits();
	SetKey(REG_KEY_CON, _T("FixedDigits"), dwValue);


    dwValue = (DWORD)g_bTVMEndPay;
    SetKey(REG_KEY_CON, _T("TVMEndPay"), dwValue);

    dwValue = (DWORD)g_bGraphFast;
    SetKey(REG_KEY_CON, _T("GraphFast"), dwValue);

    dwValue = (DWORD)g_bGraphPoints;
    SetKey(REG_KEY_CON, _T("GraphPoints"), dwValue);

    dwValue = (DWORD)m_oCalc->GetDRGState();
    SetKey(REG_KEY_CON, _T("DRG"), dwValue);
...
BOOL CIssCalcInterface::Draw(CIssGDIEx& gdiMem, RECT& rc)
{
    ...
    else
    {
	    ...
	    m_oMenu.DrawMenu(hdc, rc, 
                szTemp, 
                m_oStr->GetText(ID(IDS_MENU_Menu), m_hInst));
    }
...
BOOL CIssCalcInterface::OnMenuRight()
{
    ...
    int iChecked = 0;
    if(m_oBtnManger.GetCalcType() == CALC_UnitConv)
        iChecked = 0;
...
void CIssCalcInterface::LaunchHelp()
{
	...
	{
	case LANG_FRENCH:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_fr.htm"));
		break;
	case LANG_GERMAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_de.htm"));
		break;
	case LANG_SPANISH:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_es.htm"));
		break;
	case LANG_JAPANESE:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_jp.htm"));
		break;
	case LANG_DUTCH:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_du.htm"));
		break;
	case LANG_KOREAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_ko.htm"));
		break;
	case LANG_ITALIAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_it.htm"));
		break;
	case LANG_PORTUGUESE:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_pt.htm"));
		break;
	case LANG_RUSSIAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter_ru.htm"));
		break;
	default:
		m_oStr->StringCopy(szHelpFile, _T("PanoConverter.htm"));
		break;
	}
...
void CIssCalcInterface::ChangeCalcModes(EnumCalcType eCalcType)
{
    ...
    if(eCalcType != CALC_Currency &&
        eCalcType != CALC_UnitConv)
        return;
...
void CIssCalcInterface::UpdateWindowText()
{
    ...
    */
    ...
    case CALC_SciRPN:
        m_oStr->StringCopy(szWindowText, ID(IDS_MENU_Scientific), STRING_LARGE, m_hInst);
        break;
    case CALC_Financial:
...
BOOL CIssCalcInterface::NextCalc(BOOL bForward)
{
    ...
    eNextCalc = m_oBtnManger.GetCalcType();

    if(eNextCalc == CALC_Currency)
        eNextCalc = CALC_UnitConv;
    else
        eNextCalc = CALC_Currency;
