...
,m_oInterface(FALSE)
{
	...
	if(S_OK == GetKey(REG_KEY_CON, REG_PS, dwVal))
...
CDlgCalcPro::~CDlgCalcPro(void)
{
	...
	SetKey(REG_KEY_CON, REG_PS, dwVal);
...
//        HWND hWndSip = FindWindow(_T("MS_SIPBUTTON"), NULL);
  //      ShowWindow(hWndSip, SW_SHOW);
    ...
    m_oKey.Init(_T("SOFTWARE\\Panoramic\\PanoConverter"), _T("SOFTWARE\\Pano\\PCC2"), 0x0, 0x2CB2BFCC, 0x86345EEF, 4, TRUE);
    m_oKey.CheckSetKey();
    m_oKey.CheckDemo();
...
BOOL CDlgCalcPro::OnInitDialog(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    g_cLocale.Init(REG_KEY, m_hInst, SUPPORT_English|SUPPORT_Dutch|SUPPORT_German|SUPPORT_Dutch|SUPPORT_Spanish|SUPPORT_Portuguese|SUPPORT_Italian|SUPPORT_French|SUPPORT_Japanese, TRUE);
