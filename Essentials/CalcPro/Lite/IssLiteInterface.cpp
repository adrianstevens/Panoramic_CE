#include "IssLiteInterface.h"
#include "IssRegistry.h"
#include "DlgLiteOptions.h"
#include "DlgLiteAbout.h"
#include "Resource.h"
#include "IssGDIFx.h"
...
void CIssCalcInterface::LoadRegistry()
{
    ...
    for(int i = 0; i < NUMBER_OF_MemoryBanks; i++)
    {
        ...
        if(S_OK == GetKey(REG_KEY_LITE, szMemLabel, szTemp, STRING_MAX))
...
void CIssCalcInterface::SaveRegistry()
{
    ...
    SetKey(REG_KEY_LITE, _T("UseTape"), dwValue);

    for(int i = 0; i < NUMBER_OF_MemoryBanks; i++)
    {
        m_oStr->Format(szMemLabel, _T("Memory%i"), i);
        m_oCalc->GetMemory(szTemp, i);
        SetKey(REG_KEY_LITE, szMemLabel, szTemp, m_oStr->GetLength(szTemp));
    }

	dwValue = (DWORD)m_oBtnManger.GetCalcType();
	SetKey(REG_KEY_LITE, _T("CalcType"), dwValue);

	dwValue = (DWORD)m_eCalcMode;
	SetKey(REG_KEY_LITE, _T("CalcMode"), dwValue);

	dwValue = (DWORD)m_oCalc->GetRPNStackSize();
	SetKey(REG_KEY_LITE, _T("RPNStack"), dwValue);

	dwValue = (DWORD)m_oCalc->GetCalcDisplay();
	SetKey(REG_KEY_LITE, _T("Notation"), dwValue);

	dwValue = (DWORD)m_oCalc->GetBaseBits();
	SetKey(REG_KEY_LITE, _T("BaseBits"), dwValue);

	dwValue = (DWORD)m_bUseRegionalSettings;
	SetKey(REG_KEY_LITE, _T("RegionalSettings"), dwValue);

    dwValue = (DWORD)m_oBtnManger.GetUseAdvMem();
    SetKey(REG_KEY_LITE, _T("AdvMem"), dwValue);

	dwValue = (DWORD)m_bPlaySounds;
	SetKey(REG_KEY_LITE, _T("PlaySounds"), dwValue);

	dwValue = (DWORD)m_oBtnManger.GetSkinType();
	SetKey(REG_KEY_LITE, _T("Skin"), dwValue);

	dwValue = (DWORD)m_bAnimate2ndHyp;
	SetKey(REG_KEY_LITE, _T("Ani2ndHyp"), dwValue);

	dwValue = (DWORD)m_oCalc->GetFixedDecDigits();
	SetKey(REG_KEY_LITE, _T("FixedDigits"), dwValue);


    dwValue = (DWORD)g_bGraphFast;
    SetKey(REG_KEY_LITE, _T("GraphFast"), dwValue);

    dwValue = (DWORD)g_bGraphPoints;
    SetKey(REG_KEY_LITE, _T("GraphPoints"), dwValue);

    dwValue = (DWORD)m_oCalc->GetDRGState();
    SetKey(REG_KEY_LITE, _T("DRG"), dwValue);
...
BOOL CIssCalcInterface::OnSize(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    if(oInt != NULL)
    {
        ...
        sInit.oMenu     = &m_oMenu;
        sInit.oConstants= NULL; //&m_oConstants;
...
void CIssCalcInterface::LaunchHelp()
{
	...
	{
	case LANG_FRENCH:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_fr.htm"));
		break;
	case LANG_GERMAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_de.htm"));
		break;
	case LANG_SPANISH:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_es.htm"));
		break;
	case LANG_JAPANESE:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_jp.htm"));
		break;
	case LANG_DUTCH:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_du.htm"));
		break;
	case LANG_KOREAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_ko.htm"));
		break;
	case LANG_ITALIAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_it.htm"));
		break;
	case LANG_PORTUGUESE:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_pt.htm"));
		break;
	case LANG_RUSSIAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite_ru.htm"));
		break;
	default:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcProLite.htm"));
		break;
	}
...
void CIssCalcInterface::ChangeCalcModes(EnumCalcType eCalcType)
{
    ...
    if(eCalcType != CALC_Standard && eCalcType != CALC_Currency)
        eCalcType = CALC_Currency;//done and done
...
void CIssCalcInterface::UpdateWindowText()
{
    ...
    */
    ...
    m_oStr->Insert(szWindowText, _T("Calc Pro Lite - "));
...
{
    //animate here??
    EnumCalcType eCalc = m_oBtnManger.GetPrevCalc();

    if(eCalc != CALC_Standard && eCalc != CALC_Currency)
        eCalc = CALC_Currency;
    
    ChangeCalcModes(eCalc);
    return TRUE;
}
