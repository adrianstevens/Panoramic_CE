...
CDlgLite::CDlgLite(HINSTANCE hInst)
:m_bPlaySounds(TRUE)
,m_oInterface(FALSE)
{
	...
	if(S_OK == GetKey(REG_KEY_LITE, REG_PS, dwVal))
...
CDlgLite::~CDlgLite(void)
{
	// write values to the registry
	DWORD dwVal;
	dwVal		= (DWORD)m_bPlaySounds;
	SetKey(REG_KEY_LITE, REG_PS, dwVal);

   Destroy();
}

BOOL CDlgLite::Destroy()
{
	if(m_oInterface)
	{
		delete m_oInterface;
...
BOOL CDlgLite::OnSize(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	RECT rcClient;
...
BOOL CDlgLite::OnActivate(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    BOOL bRet = UNHANDLED;
#ifdef UNDER_CE
    HWND hWndSip = NULL;
...
//        HWND hWndSip = FindWindow(_T("MS_SIPBUTTON"), NULL);
  //      ShowWindow(hWndSip, SW_SHOW);
    ...
    m_oKey.Init(_T("SOFTWARE\\Panoramic\\PanoCalcProLite"), _T("SOFTWARE\\Pano\\PCPL2"), 0x0, 0x2B00B1ED, 0x6498ABC1, 4, TRUE);
...
BOOL CDlgLite::OnInitDialog(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	::SetCursor(LoadCursor(NULL, IDC_WAIT));

    //let's get iSSLocalisation going
	m_oStr->SetResourceInstance(m_hInst);
    g_cLocale.Init(REG_KEY, m_hInst, SUPPORT_English|SUPPORT_German|SUPPORT_Spanish|SUPPORT_Portuguese|SUPPORT_Italian|SUPPORT_French|SUPPORT_Japanese, TRUE);
...
BOOL CDlgLite::OnHotKey(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
#ifdef WIN32_PLATFORM_WFSP
...
BOOL CDlgLite::OnPaint(HWND hWnd, HDC hDC, RECT& rcClient)
{
	RECT rcClip;
...
BOOL CDlgLite::OnChar(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
...
BOOL CDlgLite::OnUser(HWND hWnd, UINT uiMessage, WPARAM wParam, LPARAM lParam)
{
    if(m_oKey.m_bGood == FALSE)
    {
...
BOOL CDlgLite::OnLButtonDown(HWND hWnd, POINT& pt)
{
	if(m_oInterface)
...
BOOL CDlgLite::OnLButtonUp(HWND hWnd, POINT& pt)
{	
	if(m_oInterface)
...
BOOL CDlgLite::OnMouseMove(HWND hWnd, POINT& pt)
{
	if(m_oInterface)
...
BOOL CDlgLite::OnKeyDown(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	if(m_oInterface)
...
BOOL CDlgLite::OnKeyUp(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	if(m_oInterface)
...
BOOL CDlgLite::OnHandleNavBtn(WPARAM wParam)
{
	switch(wParam)
	{
	case VK_LEFT:
...
BOOL CDlgLite::OnTimer(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	if(m_oInterface != NULL)
		return m_oInterface->OnTimer(hWnd, wParam, lParam);
	return FALSE;
}
BOOL CDlgLite::OnDestroy(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	return CIssWnd::OnDestroy(hWnd, wParam, lParam);
}
BOOL CDlgLite::OnCommand(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	if(m_oInterface != NULL)
...
void CDlgLite::PlaySounds(TCHAR* szWave)
{
	if(m_bPlaySounds)
