...
#else
    m_hFont = CIssGDIEx::CreateFont(24, FW_BOLD, TRUE);
#endif
}
...
BOOL CDlgAbout::DrawAboutText(HDC hdc, RECT rcClip)
{
    ...
    {
    case RS_Full:
        m_oStr->StringCopy(szState, m_oStr->GetText(ID(IDS_INFO_RetailVersion), m_hInst));
    	break;
    case RS_Demo:
        m_oStr->Format(szState, m_oStr->GetText(ID(IDS_INFO_DemoDays), m_hInst), m_iDaysRemaining);
    	break;
    case RS_Expired:
    default:
        m_oStr->StringCopy(szState, m_oStr->GetText(ID(IDS_INFO_Expired), m_hInst));
        break;
    }
    
    m_oStr->Format(szTemp, m_oStr->GetText(ID(IDS_MSG_About), m_hInst), szState);
...
BOOL CDlgAbout::OnMove(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	RECT rcWindow;
