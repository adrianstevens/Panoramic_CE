...
BOOL                 g_bGraphFast;
BOOL                 g_bTVMEndPay;
PAYMENT_SCHEDULE     g_ePaymentSched;
EnumCalcSizes		 g_eCalcSize;
...
void CIssCalcInterface::Destroy()
{
    ...
    CIssGDIEx::DeleteFont(m_hFontBtnLabels);
...
void CIssCalcInterface::LoadRegistry()
{
    ...
    if(S_OK == GetKey(REG_KEY, _T("MortPaymentSched"), dwTemp))
        g_ePaymentSched = (PAYMENT_SCHEDULE)dwTemp;
    else
        g_ePaymentSched = PAYMENT_MONTHLY;
	...
	if(S_OK == GetKey(REG_KEY, _T("Size"), dwTemp))
		g_eCalcSize = ((EnumCalcSizes)dwTemp);
    else g_eCalcSize = CALCSIZE_Large;

    if(S_OK == GetKey(REG_KEY, _T("KBType"), dwTemp))
        m_oBtnManger.SetKBType((EnumKeyboard)dwTemp);

    if(S_OK == GetKey(REG_KEY, _T("KBLoc"), dwTemp))
        m_oBtnManger.SetKBOffset((int)dwTemp);

    if(S_OK == GetKey(REG_KEY, _T("KBLetters"), dwTemp))
        m_oBtnManger.SetKBLetters((BOOL)dwTemp);
...
void CIssCalcInterface::SaveRegistry()
{
    ...
    dwValue = (DWORD)g_ePaymentSched;
    SetKey(REG_KEY, _T("MortPaymentSched"), dwValue);
	...
	dwValue = (DWORD)g_eCalcSize;
	SetKey(REG_KEY, _T("Size"), dwValue);

    dwValue = (DWORD)m_oBtnManger.GetKBType();
    SetKey(REG_KEY, _T("KBType"), dwValue);

    dwValue = (DWORD)m_oBtnManger.GetKBOffset();
    SetKey(REG_KEY, _T("KBLoc"), dwValue);

    dwValue = (DWORD)m_oBtnManger.GetKBLetters();
    SetKey(REG_KEY, _T("KBLetters"), dwValue);

    //so it saves the tape if required
    m_oCalc->SetUseTape(bTemp);

#ifndef UNDER_CE
	//and for PC ... we'll save the window position
	RECT rcWindow;
	GetWindowRect(m_hWndDlg, &rcWindow);

	dwValue = (DWORD)rcWindow.left;
	SetKey(REG_KEY, _T("XPos"), dwValue);
	dwValue = (DWORD)rcWindow.top;
	SetKey(REG_KEY, _T("YPos"), dwValue);
#endif
...
BOOL CIssCalcInterface::Draw(CIssGDIEx& gdiMem, RECT& rc)
{
    ...
    {
    case CALC_UnitConv:
        m_oUnitInt.Draw(gdiMem, rc);
        DrawSelector(hdc, rc);
    	break;
    case CALC_Currency:
    	m_oCurInt.Draw(gdiMem, rc);
        DrawSelector(hdc, rc);
        break;
    case CALC_DateTime:
        m_oDateInt.Draw(hdc, rc);    
        break;
    case CALC_Graphing:
        DrawBackground(hdc, rc);
        m_oGraphInt.Draw(hdc, rc);
        DrawButtons(gdiMem, rc);
        DrawSelector(hdc, rc);
        break;
    case CALC_Constants:
        DrawBackground(hdc, rc);
        m_oConstInt.Draw(hdc, rc);
        if(m_oConstInt.DrawMainButtons())
            DrawButtons(gdiMem, rc);
        DrawSelector(hdc, rc);
        DrawDisplay(hdc, rc);
        break;
...
BOOL CIssCalcInterface::IsMenuUp()
{
	if(m_wndMenu.IsWindowUp())
		return TRUE;
...
void CIssCalcInterface::DrawDisplay(HDC hDC, RECT& rc)
{
	...
	if(m_oCalc->GetCalcBase() == 10 && m_oCalc->GetShowBCD() == FALSE)
	{
		...
		case DISPLAY_Engineering:
			m_oStr->StringCopy(szTemp, m_oStr->GetText(ID(IDS_OPT_ENG), m_hInst));
			break;
...
void CIssCalcInterface::DrawSelector(HDC hDC, RECT& rc)
{
    ...
    case CALC_Currency:
        ...
        return;
    case CALC_UnitConv:
        m_oUnitInt.DrawSelector(hDC, rc);
        return;
        break;
    default:
        break;
...
void CIssCalcInterface::DrawMainButtons(HDC hDC, RECT& rc)
{
        ...
        for(i = 0; i < sLayout->iMainBtnColumns; i++)
        {
            for(j = 0; j < sLayout->iMainBtnRows; j++)
            {
                ...
                if(oBtn)
                {
                    ...
                    /*	if(GetDeviceType() == DEVTYPE_SPLand) BUGSP
                    continue;
...
void CIssCalcInterface::DrawAltButtons(HDC hDC, RECT& rc)
{
    LayoutCalcType* sLayout = m_oBtnManger.GetCurrentLayout();
    CIssVector<ButtonType>* arrBtnType = &m_oBtnManger.GetCurrentLayout()->sMainButtons;
...
void CIssCalcInterface::DrawKeyboardButtons(HDC hDC, RECT& rc)
{
    LayoutCalcType* sLayout = m_oBtnManger.GetCurrentLayout();
    CIssVector<ButtonType>* arrBtnType = &m_oBtnManger.GetCurrentLayout()->sKeyboardButtons;
    ...
    for(int i = 0; i < sLayout->sKeyboardButtons.GetSize(); i++)
    {
        ...
        if(oBtn)
        {
            ...
            if(m_oBtnManger.GetKBLetters())
                ::DrawText(hDC, m_oBtnManger.GetKeyName(i), rcTemp, DT_TOP | DT_RIGHT, m_hFontBtnLabels, m_oBtnManger.GetSkin()->GetButton((*arrBtnType)[iButton]->iSkinIndex)->TextColor);
...
void CIssCalcInterface::DrawKeyboardButtonsAlt(CIssGDIEx& gdiMem, RECT& rc)
{
    LayoutCalcType* sLayout = m_oBtnManger.GetCurrentLayout();
    CIssVector<ButtonType>* arrBtnType = &m_oBtnManger.GetCurrentLayout()->sKeyboardButtons;
    ...
    for(int i = 0; i < sLayout->sKeyboardButtons.GetSize(); i++)
    {
        ...
        if(oBtn)
        {
            ...
            rcTemp.right -= GetSystemMetrics(SM_CXSMICON)/6;
            if(m_oBtnManger.GetKBLetters())
                ::DrawText(gdiMem.GetDC(), m_oBtnManger.GetKeyName(i), rcTemp, DT_TOP | DT_RIGHT, m_hFontBtnLabels, 0xFFFFFF);
            ...
            if((*arrBtnType)[iButton]->iButtonType == INPUT_Clear &&
                (*arrBtnType)[iButton]->iButtonValue == CLEAR_BackSpace)
            {
                DrawBackSpace(gdiMem.GetDC(), oBtn->GetButtonRect(), 
                    m_oBtnManger.GetSkin()->GetButton((*arrBtnType)[iButton]->iSkinIndex)->TextColor,
                    m_oBtnManger.GetSkin()->GetButton((*arrBtnType)[iButton]->iSkinIndex)->TextColor2,
                    m_oBtnManger.GetSkin()->m_bUseTextShadow);
            }
...
void CIssCalcInterface::DrawButtons(CIssGDIEx& gdiMem, RECT& rc)
{
    /*
#ifdef WIN32_PLATFORM_WFSP
    if(GetSystemMetrics(SM_CXSCREEN) > GetSystemMetrics(SM_CYSCREEN))
        DrawKeyboardButtons(hDC, rc);
    else
#endif */

    HDC hDC = gdiMem.GetDC();

    if(m_oBtnManger.GetKBType() != KB_Original)
        DrawKeyboardButtonsAlt(gdiMem, rc);
    else
        DrawMainButtons(hDC, rc);
    DrawAltButtons(hDC, rc);
}
...
BOOL CIssCalcInterface::OnSize(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    m_oUnitInt.OnSize(hWnd, wParam, lParam);
    m_oCurInt.OnSize(hWnd, wParam, lParam);
    m_oDateInt.OnSize(hWnd, wParam, lParam);
...
#ifdef WIN32_PLATFORM_WFSP
#define MULT_RATIO 19/12// 3/2  BUGSP
#else
#define MULT_RATIO 2
#endif
...
BOOL CIssCalcInterface::InitDisplay()
{
	...
	if(bWideOrSquare)
	{
		...
		{	//Landscape
			...
			m_iYAltStart				= DISPLAY_HEIGHT - DISPLAY_INDENT;	//DISPLAY_HEIGHT*4/3 - DISPLAY_INDENT;			//start the alt buttons under the display
		...
		else //square
		{
			...
			m_rcDisplayArea.left		= DISPLAY_INDENT;
			m_rcDisplayArea.right		= rcClient.right-DISPLAY_INDENT;
			m_rcDisplayArea.top			= DISPLAY_INDENT;
			m_rcDisplayArea.bottom		= m_iYAltStart;
		}
	}
...
BOOL CIssCalcInterface::InitKeyboardButtons()
{
    RECT rcClient;
    GetClientRect(m_hWndDlg, &rcClient);
    ...
    //gotta set this depending on the alt buttons
    if(m_rcAltButtons.left == 0)
    {
        m_rcMainButtons = rcClient;
        m_rcMainButtons.top = m_rcAltButtons.bottom;
        m_rcMainButtons.bottom -= m_oMenu.GetMenuHeight();
    }
    else
    {
        m_rcMainButtons.top = m_rcDisplayArea.bottom;
        m_rcMainButtons.bottom = m_rcAltButtons.bottom;
        m_rcMainButtons.right = m_rcAltButtons.left;
        m_rcMainButtons.left = rcClient.left;
    }


    iButtonWidth = (WIDTH(m_rcMainButtons) - 2*BTN_EDGE_INDENT - 9*BTN_SPACING)/10;
    iButtonHeight = (HEIGHT(m_rcMainButtons) - BTN_EDGE_INDENT - 2*BTN_SPACING)/3; 
    ...
    switch(m_oBtnManger.GetSkinType())
    {
    case SKIN_Pink:
    case SKIN_Blue:    
        iSkinIndex = 1;
    ...
    for(int i = 0; i < 10; i++) //three rows at once
    {
        ...
        rcButtonSize.top    = BTN_SPACING + m_rcMainButtons.top;
        ...
        InitButton(oBtn, &rcButtonSize, oSkin, iSkinIndex, iButtonWidth, iButtonHeight, TRUE);
...
{	
	...
	//Main Buttons First
	if(bWide)
		m_rcMainButtons.right		= m_rcAltButtons.left;//HEIGHT(rcClient);
	else
...
{	
    ...
    if(WIDTH(rcClient) > HEIGHT(rcClient))
        bWide = TRUE;
    else if(WIDTH(rcClient) == HEIGHT(rcClient))//BUGBUG
        bSquare = TRUE;
	...
	if(bWide)//we're in landscape
    {
        ...
        iAreaHeight			= (m_rcSize.bottom - m_rcSize.top);
...
#endif 
	{
        ...
        if(bWide)
        {
            iAreaHeight         = (HEIGHT(m_rcSize) - HEIGHT(m_rcDisplayArea) - m_oMenu.GetMenuHeight() - BTN_SPACING)/2;
        //    iAreaHeight			= (m_rcSize.bottom - m_rcSize.top - GetSystemMetrics(SM_CXICON)3*/2)-HEIGHT(m_rcDisplayArea) - m_oMenu.GetMenuHeight() - TEXT_INDENT;
		...
		m_rcAltButtons.right= rcClient.right;
		m_rcAltButtons.top	= m_iYAltStart;
		m_rcAltButtons.bottom= m_iYAltStart + iAreaHeight;
	}
...
#ifndef WIN32_PLATFORM_WFSP  //BUGSP ... leave this one I think
  ...
  if(m_hFontAltBtnText == FALSE)
            m_hFontAltBtnText = CIssGDIEx::CreateFont(iTemp*8/10, FW_NORMAL, TRUE);
        if(m_hFontAltLongText == FALSE)
            m_hFontAltLongText = CIssGDIEx::CreateFont(iTemp*8/10, FW_NORMAL, TRUE);
        if(m_hFontAltXLText == NULL)
            m_hFontAltXLText = CIssGDIEx::CreateFont(iTemp*8/10, FW_NORMAL, TRUE);
#else
        if(m_hFontAltBtnText == FALSE)
...
//destroys the old, sets the colors, checks for a previous GDI, creates the button and saves the GDI if necessary
BOOL CIssCalcInterface::InitButton(CCalcDynBtn* oDynButton, RECT* rc, CalcProSkins* oSkin, int iSkinIndex, int iWidth, int iHeight, BOOL bNoDropShad)
{
    BOOL bShadow = oSkin->GetUseShadow();
    if(bNoDropShad)
        bShadow = FALSE;

	oDynButton->Destroy();
	oDynButton->SetTextColors(oSkin->m_typeBtnClr[iSkinIndex].TextColor, oSkin->m_typeBtnClr[iSkinIndex].TextColor2, bShadow);
...
BOOL CIssCalcInterface::OnMenuAbout()
{
	CDlgAbout dlgAbout;
	...
	dlgAbout.Init(&m_oMenu, m_hWndDlg);

	if(dlgAbout.Create(m_oStr->GetText(ID(IDS_MENU_AboutCalcPro), m_hInst), NULL, m_hInst, m_oStr->GetText(ID(IDS_MENU_About2), m_hInst),
...
BOOL CIssCalcInterface::OnMenuLeft()
{
	...
	sMenu->uiMenu = i++;
    m_wndMenu.AddItem(sMenu, IDMENU_Copy);

    m_oStr->StringCopy(szText, ID(IDS_MENU_Paste), STRING_MAX, m_hInst);
	sMenu = new TypeMenu;
	sMenu->szMenu = m_oStr->CreateAndCopy(szText);
	sMenu->uiMenu = i++;
    m_wndMenu.AddItem(sMenu, IDMENU_Paste);
    ...
    if(m_oCalc->GetUseTape() == TRUE)
    {
		...
		sMenu->szMenu = m_oStr->CreateAndCopy(szText);
		sMenu->uiMenu = i++;
        m_wndMenu.AddItem(sMenu, IDMENU_ViewTape);
    }

    m_oStr->StringCopy(szText, ID(IDS_MENU_UpdateCurrencies), STRING_MAX, m_hInst);
	sMenu = new TypeMenu;
	sMenu->szMenu = m_oStr->CreateAndCopy(szText);
	sMenu->uiMenu = i++;
    m_wndMenu.AddItem(sMenu, IDMENU_UpdateCurrencies);
    ...
    sMenu->szMenu = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu = i++;
    m_wndMenu.AddItem(sMenu, IDMENU_MoreApps);
#endif
#endif
	...
	sMenu->szMenu = m_oStr->CreateAndCopy(szText);
	sMenu->uiMenu = i++;
    m_wndMenu.AddItem(sMenu, IDMENU_Exit);
    
    m_wndMenu.SetSelectedItemIndex(0, TRUE);
	m_wndMenu.SetCustomDrawFunc(DrawMenuItem, this);
...
BOOL CIssCalcInterface::OnMenuRight()
{
    ...
    m_oStr->StringCopy(szText, ID(IDS_MENU_Standard), STRING_MAX, m_hInst);
    TypeMenu* sMenu = new TypeMenu;
    sMenu->szMenu   = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu   = 0;
    ...
    sMenu->szMenu   = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu   = 1;
    m_wndMenu.AddItem(sMenu, IDMENU_Scientific, ((m_oBtnManger.GetCalcType()==CALC_Scientific||m_oBtnManger.GetCalcType()==CALC_SciRPN)?FLAG_Radio:NULL));
    ...
    m_oStr->StringCopy(szText, ID(IDS_MENU_Graphing), STRING_MAX, m_hInst);
    sMenu = new TypeMenu;
    sMenu->szMenu   = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu   = 3;
    m_wndMenu.AddItem(sMenu, IDMENU_Graph, (m_oBtnManger.GetCalcType()==CALC_Graphing?FLAG_Radio:NULL));
    
    m_oStr->StringCopy(szText, ID(IDS_MENU_Unit), STRING_MAX, m_hInst);
    sMenu = new TypeMenu;
    sMenu->szMenu   = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu   = 4;
    m_wndMenu.AddItem(sMenu, IDMENU_Unit, (m_oBtnManger.GetCalcType()==CALC_UnitConv?FLAG_Radio:NULL));

    m_oStr->StringCopy(szText, ID(IDS_MENU_Currency), STRING_MAX, m_hInst);
    sMenu = new TypeMenu;
    sMenu->szMenu   = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu   = 5;
...
#ifndef WIN32_PLATFORM_WFSP
    m_oStr->StringCopy(szText, ID(IDS_MENU_DateTime), STRING_MAX, m_hInst);
    sMenu = new TypeMenu;
    sMenu->szMenu   = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu   = 6;
    m_wndMenu.AddItem(sMenu, IDMENU_DateTime, (m_oBtnManger.GetCalcType()==CALC_DateTime?FLAG_Radio:NULL));
#endif
    ...
    m_oStr->StringCopy(szText, ID(IDS_MENU_Financial), STRING_MAX, m_hInst);
    sMenu = new TypeMenu;
    sMenu->szMenu   = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu   = 8;
    m_wndMenu.AddItem(sMenu, IDMENU_Financial, (m_oBtnManger.GetCalcType()==CALC_Financial?FLAG_Radio:NULL));

    m_oStr->StringCopy(szText, ID(IDS_MENU_Stats), STRING_MAX, m_hInst);
    sMenu = new TypeMenu;
    sMenu->szMenu   = m_oStr->CreateAndCopy(szText);
    sMenu->uiMenu   = 9;
    m_wndMenu.AddItem(sMenu, IDMENU_Stats, (m_oBtnManger.GetCalcType()==CALC_Statistics?FLAG_Radio:NULL));

    int iChecked = 0;
    if(m_oBtnManger.GetCalcType() == CALC_Standard)
        iChecked = 0;
    else if(m_oBtnManger.GetCalcType() == CALC_Scientific || m_oBtnManger.GetCalcType() == CALC_SciRPN)
        iChecked = 1;
    else if(m_oBtnManger.GetCalcType() == CALC_BaseConversions)
        iChecked = 2;
    else if(m_oBtnManger.GetCalcType() == CALC_Graphing)
        iChecked = 3;
    else if(m_oBtnManger.GetCalcType() == CALC_UnitConv)
        iChecked = 4;
        ...
        iChecked = 6;
    else if(m_oBtnManger.GetCalcType() == CALC_Financial)
        iChecked = 7;
    else if(m_oBtnManger.GetCalcType() == CALC_Statistics)
        iChecked = 8;
#else
    else if(m_oBtnManger.GetCalcType() == CALC_DateTime)
        iChecked = 6;
    else if(m_oBtnManger.GetCalcType() == CALC_Constants)
        iChecked = 7;
    else if(m_oBtnManger.GetCalcType() == CALC_Financial)
...
#ifndef UNDER_CE
        ,ADJUST_Bottom    
#endif //this might still work on PPC
       );
...
BOOL CIssCalcInterface::OnUser(HWND hWnd, UINT uiMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CALCENGINE:
        ...
        if(wParam == (int)INPUT_Launch)//launch buttons yo
        {
            ...
            m_oWSMan.LaunchWorkSheet((LaunchType)lParam, &sInit, &m_oBtnHandler, m_bPlaySounds);

            //just pump an onsize if we've changed orientations during the Worksheet
...
BOOL CIssCalcInterface::OnCommand(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	if(m_oUnitInt.OnCommand(hWnd, wParam, lParam) == TRUE)
		return TRUE;
    if(m_oCurInt.OnCommand(hWnd, wParam, lParam) == TRUE)
        return TRUE;
    if(m_oDateInt.OnCommand(hWnd, wParam, lParam) == TRUE)
        return TRUE;
	...
	case IDMENU_MoreApps:
		OnMenuMoreApps();
		break;
    case IDMENU_UpdateCurrencies:
        m_oCurInt.OnBtnUpdate();
        break;
	case IDMENU_Options:
		OnMenuOptions();
		break;
	...
	case IDMENU_Graph:
		...
		break;
	case IDMENU_Unit:
		ChangeCalcModes(CALC_UnitConv);
		break;
    case IDMENU_Currency:
        ChangeCalcModes(CALC_Currency);
        break;
    case IDMENU_DateTime:
        ChangeCalcModes(CALC_DateTime);
        break;
    case IDMENU_Stats:
        ChangeCalcModes(CALC_Statistics);
        break;
	case IDMENU_Financial:
        ChangeCalcModes(CALC_Financial);
		break;
...
void CIssCalcInterface::LaunchHelp()
{
	...
	{
	case LANG_FRENCH:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_fr.htm"));
		break;
	case LANG_GERMAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_de.htm"));
		break;
	case LANG_SPANISH:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_es.htm"));
		break;
	case LANG_JAPANESE:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_jp.htm"));
		break;
	case LANG_DUTCH:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_du.htm"));
		break;
	case LANG_KOREAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_ko.htm"));
		break;
	case LANG_ITALIAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_it.htm"));
		break;
	case LANG_PORTUGUESE:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_pt.htm"));
		break;
	case LANG_RUSSIAN:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro_ru.htm"));
		break;
	default:
		m_oStr->StringCopy(szHelpFile, _T("PanoCalcPro.htm"));
		break;
	}
#ifdef UNDER_CE
	if(IsSmartphone())
...
BOOL CIssCalcInterface::OnLButtonDown(HWND hWnd, POINT& pt)
{
    ...
    {
    case CALC_UnitConv:
        bRet = m_oUnitInt.OnLButtonDown(hWnd, pt);
    	break;
    case CALC_Currency:
        bRet = m_oCurInt.OnLButtonDown(hWnd, pt);
        break;
    case CALC_DateTime:
        bRet = m_oDateInt.OnLButtonDown(hWnd, pt);
        break;
    case CALC_Graphing:
    ...
    else if(PtInRect(&m_rcM1, pt))
    {
        ...
        if(m_oBtnManger.GetUseAdvMem())
        {
            ...
            for(int i = 0; i < NUMBER_OF_MemoryBanks; i++)
            {
                ...
                m_oStr->Format(szTemp,m_oStr->GetText(ID(IDS_MSG_Memory), m_hInst), i, szMemory);
                m_oStr->Concatenate(szBody, szTemp);
            }
            MessageBox(m_hWndDlg, szBody, m_oStr->GetText(ID(IDS_MSG_CalcProMemory), m_hInst), MB_OK);
        }
        else if(m_oCalc->IsMemoryFilled(0))
        {
            m_oCalc->GetMemory(szMemory, 0);
            m_oStr->Format(szBody,m_oStr->GetText(ID(IDS_MSG_Memory2), m_hInst), szMemory);
            MessageBox(m_hWndDlg, szBody, m_oStr->GetText(ID(IDS_MSG_CalcProMemory), m_hInst), MB_OK);
        }
...
BOOL CIssCalcInterface::OnLButtonUp(HWND hWnd, POINT& pt)
{
    ...
    {
    case CALC_UnitConv:
        return m_oUnitInt.OnLButtonUp(hWnd, pt);
	    break;
    case CALC_Currency:
        return m_oCurInt.OnLButtonUp(hWnd, pt);
	    break;
    case CALC_DateTime:
        return m_oDateInt.OnLButtonUp(hWnd, pt);
        break;
    case CALC_Graphing:
    ...
    for(i = 0; i < m_arrMainButtons.GetSize(); i++)
    {
	    ...
	    if(oBtn && oBtn->OnLButtonUp(pt))
	    {
            if(m_oBtnManger.GetKBType() != KB_Original)
            {
                DWORD dwType = m_oBtnManger.GetCurrentLayout(NULL)->sKeyboardButtons[i]->iButtonType;
                DWORD dwID = m_oBtnManger.GetCurrentLayout(NULL)->sKeyboardButtons[i]->iButtonValue;
...
void CIssCalcInterface::UpdateWindowText()
{
    ...
    */
    ...
    m_oStr->Insert(szWindowText, m_oStr->GetText(ID(IDS_TITLE_CalcPro), m_hInst));
...
BOOL CIssCalcInterface::OnChar(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	if(m_oBtnHandler.OnChar(hWnd, wParam, lParam, m_oCalc, &m_oBtnManger))
	{
        ...
        {
        case CALC_UnitConv:
            m_oUnitInt.OnChar(hWnd, wParam, lParam);
        	break;
        case CALC_Currency:
...
BOOL CIssCalcInterface::OnMenuOptions()
{
    ...
    sOptions.ePaymentSched          = g_ePaymentSched;


//a bit hackish
#ifdef WIN32_PLATFORM_WFSP
    MSG stMsg = { 0 };
    while( PeekMessage( &stMsg, m_hWndDlg, 0, 0, PM_REMOVE ))
    {
	...
	if(dlgOptions.Create(m_oStr->GetText(ID(IDS_MENU_CalcProOptions), m_hInst), NULL, m_hInst, m_oStr->GetText(ID(IDS_MENU_Options2), m_hInst),
		CW_USEDEFAULT,CW_USEDEFAULT,CW_USEDEFAULT,CW_USEDEFAULT,
		0,
	...
	if(IDOK == dlgOptions.GetReturn())
#endif
    {
        sOptions = dlgOptions.GetOptions();

		// if the language has changed
		if(dlgOptions.GetOriginalLanguage() != g_cLocale.GetCurrentLanguage())
		{
			// have to reload the unit converter text
			m_oUnitInt.GetConverter().ReloadUnitTextItems();
		}
            ...
            m_oBtnManger.GetFullscreen() != sOptions.bFullScreen)
	    {
            ...
            SendMessage(m_hWndDlg, WM_ACTIVATE, WA_CLICKACTIVE, 0);
            ...
            m_oUnitInt.Init(NULL);
            m_oCurInt.Init(NULL);
            m_oDateInt.Init(NULL);
            m_oGraphInt.Init(NULL);
            m_oConstInt.Init(NULL);
            
		    Draw(gdiCur, rc);//draw the new skin to our gdi

		    if(S_OK != AlphaTransition(hDC, rc, gdiPrev, rc, gdiCur, rc))
				InvalidateRect(m_hWndDlg, NULL, FALSE);
...
{
    //since date/time is the only one that needs this I'm only going to handle the one for now
    if(m_oBtnManger.GetCalcType() == CALC_DateTime)
        m_oDateInt.LostFocus();
    else if(m_oBtnManger.GetCalcType() == CALC_UnitConv)
        m_oUnitInt.LostFocus();
}
...
BOOL CIssCalcInterface::NextCalc(BOOL bForward)
{
    ...
    if(bForward) 
    {
        ...
        if(eNextCalc >= CALC_WorkSheet)
            eNextCalc = (EnumCalcType)0;
        else if(eNextCalc == CALC_SciRPN)
            eNextCalc = (EnumCalcType)(CALC_SciRPN+1);
        else if(eNextCalc == CALC_Scientific &&
    ...
    {   //back
        ...
        if(eNextCalc < 0)
            eNextCalc = (EnumCalcType)(CALC_WorkSheet - 1);//just easier
            ...
            eNextCalc = (EnumCalcType)(CALC_Scientific - 1);
    }
...
#else
    if(bForward)
        SlideLeft(dc, gdiNext, 500);
    else
        SlideRight(dc, gdiNext, 500);
#endif
...
void CIssCalcInterface::DrawIconItem(CIssGDIEx& gdi, RECT& rcDraw, TypeItems* sItem, BOOL bIsHighlighted)
{
    ...
    DrawText(gdi.GetDC(), sMenu->szMenu, rc, DT_LEFT|DT_VCENTER|DT_END_ELLIPSIS, m_hFontMenu, (bIsHighlighted?TEXTCOLOR_Selected:TEXTCOLOR_Normal));
...
void CIssCalcInterface::DrawMenuItem(CIssGDIEx& gdi, RECT& rcDraw, TypeItems* sItem, BOOL bIsHighlighted, LPVOID lpClass)
{
	CIssCalcInterface* pThis = (CIssCalcInterface*)lpClass;
	if(pThis)
		pThis->DrawMenuItem(gdi, rcDraw, sItem, bIsHighlighted);
}

void CIssCalcInterface::DrawMenuItem(CIssGDIEx& gdi, RECT& rcDraw, TypeItems* sItem, BOOL bIsHighlighted)
{
	RECT rc = rcDraw;
...
//	TCHAR szTemp[STRING_NORMAL];
	...
	DrawText(gdi, sMenu->szMenu, rc, DT_LEFT|DT_VCENTER, m_hFontMenu, TEXTCOLOR_Normal);
}
...
void CIssCalcInterface::OnMenuRPNStack()
{
    ...
    m_wndMenu.AddCategory(m_oStr->GetText(ID(IDS_MENU_RPNStack), m_hInst));
...
void CIssCalcInterface::OnMenuConstants()
{
    ...
    m_wndMenu.AddCategory(m_oStr->GetText(ID(IDS_MENU_ConstantFavorites), m_hInst));
    ...
    int iHeightSel = GetSystemMetrics(SM_CXICON)*8/5;
    m_wndMenu.SetItemHeights(iHeight, iHeightSel);
    ...
    m_wndMenu.PopupMenu(m_hWndDlg, m_hInst,
        OPTION_DrawScrollArrows|OPTION_AlwaysShowSelector|OPTION_Bounce,
        rc.left, rc.top,
        WIDTH(rc), HEIGHT(rc),
        rcBtn.left, rcBtn.top,
        WIDTH(rcBtn), HEIGHT(rcBtn),
...
void CIssCalcInterface::SetExpired(BOOL bExpired)
{
    m_bExpired = bExpired;
    m_oUnitInt.SetExpired(bExpired);
    m_oCurInt.SetExpired(bExpired);
}

void CIssCalcInterface::SetWindowSize()
{
	if(m_hFontDisplay)
		CIssGDIEx::DeleteFont(m_hFontDisplay);
	if(m_hFontExponent)
		CIssGDIEx::DeleteFont(m_hFontExponent);
	if(m_hFontDisplayTop)
		CIssGDIEx::DeleteFont(m_hFontDisplayTop);
	if(m_hFontSmallTop)
		CIssGDIEx::DeleteFont(m_hFontSmallTop);
	if(m_hFontRPN)
	...
	m_hFontSmallTop		= CIssGDIEx::CreateFont(DISPLAY_TOP_TEXT_SM, FW_NORMAL, TRUE);
	m_hFontRPN          = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXSMICON), FW_BOLD, TRUE);
    m_hFontBtnLabels    = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXSMICON)*9/16, FW_NORMAL, TRUE);
...
//    SetWindowPos(m_hWndDlg, NULL, pt.x, pt.y, 0, 0, SWP_NOSIZE | SWP_NOREDRAW);
...
//	RECT rc;
//	GetWindowRect(m_hWndDlg, &rc);	
	SetWindowPos(m_hWndDlg, NULL, pt.x, pt.y, WINDOW_WIDTH, WINDOW_HEIGHT, SWP_NOREDRAW);
	InitMenu();
#endif
}

BOOL CIssCalcInterface::IsVGA()
{
#ifdef UNDER_CE
	return  (GetSystemMetrics(SM_CXICON) > 44);
#else
	if(g_eCalcSize < CALCSIZE_Large)
		return FALSE;
	return TRUE;
#endif
