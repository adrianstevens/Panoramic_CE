...
BOOL CDlgOptions::DrawButtonItem(CIssGDIEx& gdi, BOOL bIsHighlighted, RECT& rc, EnumOption* eOption)
{
    ...
    case OPTION_EndPeriodPayments:
        m_oStr->StringCopy(szInfo, m_sOptions.bTVMEndPeriod?m_oStr->GetText(ID(IDS_MENU_On), m_hInst):m_oStr->GetText(ID(IDS_MENU_Off), m_hInst));
        break;
    case OPTION_PaymentSchedule:
        GetPaymentSchedText(m_sOptions.ePaymentSched, szInfo);
        break;
    ...
    case OPTION_FixedDigits:
        ...
        break;
	case OPTION_Size:
		GetSizeText(m_sOptions.eSize, szInfo);
		break;
    case OPTION_Keyboard:
        GetKeyboardText(m_sOptions.eKB, szInfo);
        break;
    case OPTION_KeyboardOffset:
        GetKeyboardOffsetText(m_sOptions.iKBOffset, szInfo);
        break;
    case OPTION_KeyboardLetters:
        m_oStr->StringCopy(szInfo, m_sOptions.bKBLetters?m_oStr->GetText(ID(IDS_MENU_On), m_hInst):m_oStr->GetText(ID(IDS_MENU_Off), m_hInst));
        break;
    case OPTION_FullScreen:
        m_oStr->StringCopy(szInfo, m_sOptions.bFullScreen?m_oStr->GetText(ID(IDS_MENU_On), m_hInst):m_oStr->GetText(ID(IDS_MENU_Off), m_hInst));
        break;
    }
...
void CDlgOptions::GetKeyboardText(EnumKeyboard eKB, TCHAR* szInfo)
{
    switch(eKB)
    {
    default:
    case KB_Original:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Layout), m_hInst));
    	break;
    case KB_QWERTY:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_QWERTY), m_hInst));
    	break;
    case KB_QWERTZ:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_QWERTZ), m_hInst));
        break;
    case KB_AZERTY:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_AZERTY), m_hInst));
        break;
    }
}

void CDlgOptions::GetKeyboardOffsetText(int iOffset, TCHAR* szInfo)
{
    switch(iOffset)
    {
    default:
    case 0:
        if(m_sOptions.eKB == KB_AZERTY)
            m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Z), m_hInst));
        else
            m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_W), m_hInst));
    	break;
    case 1:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_E), m_hInst));
        break;
    case 2:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_R), m_hInst));
        break;
    case 3:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_T), m_hInst));
        break;
    }
}

void CDlgOptions::GetSizeText(EnumCalcSizes eSize, TCHAR* szInfo)
{
	switch(eSize)
	{
	...
	case CALCSIZE_NormalWide:
		m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_NormalWide), m_hInst));
		break;
	case CALCSIZE_Small:
		m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Small), m_hInst));
	    break;
	case CALCSIZE_SmallWide:
		m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_SmallWide), m_hInst));
	    break;
	case CALCSIZE_Large:
		m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Large), m_hInst));
		break;
	case CALCSIZE_LargeWide:
		m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_LargeWide), m_hInst));
		break;
	}
}

void CDlgOptions::GetPaymentSchedText(PAYMENT_SCHEDULE eSched, TCHAR* szInfo)
{
    switch(eSched)
    {
    default:
    case PAYMENT_MONTHLY:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Monthly), m_hInst));
    	break;
    case PAYMENT_MONTHLY_ACCELERATED:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_MonthlyAccel), m_hInst));
    	break;
    case PAYMENT_BIWEEKLY:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_BiWeekly), m_hInst));
        break;
    case PAYMENT_BIWEEKLY_ACCELERATED:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_BiWeeklyAccel), m_hInst));
        break;
    case PAYMENT_WEEKLY:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Weekly), m_hInst));
    	break;
    case PAYMENT_WEEKLY_ACCELERATED:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_WeeklyAccel), m_hInst));
    	break;
    case PAYMENT_SEMI_MONTHLY:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_SemiMonthly), m_hInst));
        break;
    case PAYMENT_SEMI_MONTHLY_ACCELERATED:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_SemiMonthlyAccel), m_hInst));
        break;
    case PAYMENT_BIMONTHLY:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_BiMonthly), m_hInst));
        break;
    case PAYMENT_BIMONTHLY_ACCELERATED:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_BiMonthlyAccel), m_hInst));
        break;
    }
}
...
void CDlgOptions::GetItemTitle(EnumOption eOption, TCHAR* szTitle)
{
    ...
    case OPTION_GraphUsePoints:
        ...
        break;
    case OPTION_EndPeriodPayments:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_EndPeriodPayments), m_hInst));
        break;
    case OPTION_PaymentSchedule:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_PaymentSched), m_hInst));
        break;
	case OPTION_Size:
		m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_CalculatorSize), m_hInst));
		break;
    case OPTION_Keyboard:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_KeyboardType), m_hInst));
        break;
    case OPTION_KeyboardOffset:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_1Key), m_hInst));
        break;
    case OPTION_KeyboardLetters:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_KeyboardLetters), m_hInst));
        break;
	case OPTION_Language:
		g_cLocale.GetLanguageOptionText(szTitle, g_cLocale.GetCurrentLanguage());
		break;
    case OPTION_FullScreen:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_Fullscreen), m_hInst));
        break;
    }
}
...
void CDlgOptions::PopulateOptions()
{
    ...
    *eOption = OPTION_RPNStackSize;
    ...
    m_oMenu.AddCategory(m_oStr->GetText(ID(IDS_MENU_Financial), m_hInst));

    eOption = new EnumOption;
    *eOption = OPTION_EndPeriodPayments;
    m_oMenu.AddItem(eOption, IDMENU_Select);

    eOption = new EnumOption;
    *eOption = OPTION_PaymentSchedule;
    m_oMenu.AddItem(eOption, IDMENU_Select);
...
void CDlgOptions::LaunchPopupMenu()
{
    ...
    case OPTION_EndPeriodPayments:
        ...
        break;
    case OPTION_PaymentSchedule:
        for(i = 0; i < (int)PAYMENT_BIMONTHLY_ACCELERATED; i++)
        {
            PAYMENT_SCHEDULE eSched = (PAYMENT_SCHEDULE)i;
            GetPaymentSchedText(eSched, szInfo);
            m_wndMenu->AddItem(szInfo, IDMENU_SelectMenu, (m_sOptions.ePaymentSched == eSched?FLAG_Radio:NULL), eSched);
        }
        break;
    case OPTION_CalculatorMode:        
	...
	case OPTION_Size:
		for(int i = 2; i < 6; i++)
		{
			EnumCalcSizes eSize = (EnumCalcSizes)i;
			GetSizeText(eSize, szInfo);
			m_wndMenu->AddItem(szInfo, IDMENU_SelectMenu, (m_sOptions.eSize == eSize?FLAG_Radio:NULL), eSize);
		}
        break;
    case OPTION_Keyboard:
        for(int i = 1; i < 4; i++)
        {
            EnumKeyboard eKB = (EnumKeyboard)i;
            GetKeyboardText(eKB, szInfo);
            m_wndMenu->AddItem(szInfo, IDMENU_SelectMenu, (m_sOptions.eKB == eKB?FLAG_Radio:NULL), eKB);
        }
        break;
    case OPTION_KeyboardOffset:
        for(int i = 0; i < 4; i++)
        {
            GetKeyboardOffsetText(i, szInfo);
            m_wndMenu->AddItem(szInfo, IDMENU_SelectMenu, (m_sOptions.iKBOffset == i?FLAG_Radio:NULL), i);
        }
        break;
    case OPTION_KeyboardLetters:
        m_wndMenu->AddItem(m_oStr->GetText(ID(IDS_MENU_On), m_hInst), IDMENU_SelectMenu, (m_sOptions.bKBLetters?FLAG_Radio:NULL), TRUE);
        m_wndMenu->AddItem(m_oStr->GetText(ID(IDS_MENU_Off), m_hInst), IDMENU_SelectMenu, (!m_sOptions.bKBLetters?FLAG_Radio:NULL), FALSE);
        break;
    case OPTION_FullScreen:
        m_wndMenu->AddItem(m_oStr->GetText(ID(IDS_MENU_On), m_hInst), IDMENU_SelectMenu, (m_sOptions.bFullScreen?FLAG_Radio:NULL), TRUE);
        m_wndMenu->AddItem(m_oStr->GetText(ID(IDS_MENU_Off), m_hInst), IDMENU_SelectMenu, (!m_sOptions.bFullScreen?FLAG_Radio:NULL), FALSE);
        break;
    }
...
void CDlgOptions::SetMenuSelection()
{
    ...
    case OPTION_EndPeriodPayments:
        ...
        break;
    case OPTION_PaymentSchedule:
        m_sOptions.ePaymentSched = (PAYMENT_SCHEDULE)sMenuItem->lParam;
        break;
	case OPTION_Size:
		m_sOptions.eSize = (EnumCalcSizes)sMenuItem->lParam;
        break;
    case OPTION_Keyboard:
        m_sOptions.eKB = (EnumKeyboard)sMenuItem->lParam;
        break;
    case OPTION_KeyboardOffset:
        m_sOptions.iKBOffset = (int)sMenuItem->lParam;
        break;
    case OPTION_KeyboardLetters:
        m_sOptions.bKBLetters = (BOOL)sMenuItem->lParam;
        break;
    case OPTION_FullScreen:
        m_sOptions.bFullScreen = (BOOL)sMenuItem->lParam;
        break;
	}
}
...
int CDlgOptions::GetSystemMetrics(__in int nIndex)
{
