...
,m_iLastIndex(-1)
{
    ...
    m_szFavorites = m_oStr->CreateAndCopy(_T("Favorites"));
...
CObjConstants::~CObjConstants(void)
{
    Destroy();
    DeleteFavoriteArray();
    m_oStr->Delete(&m_szFavorites);
}

void CObjConstants::SetFavString(TCHAR* szText)
{
    if(szText == NULL)
        return;
    if(m_oStr->GetLength(szText) == 0)
        return;
    m_oStr->Delete(&m_szFavorites);
    m_szFavorites = m_oStr->CreateAndCopy(szText);
}
...
{
    TypeFavorite* sConst = new TypeFavorite;//Adrian said to leave this on March 11, 2010
...
HRESULT CObjConstants::PopulateGroupFavs()
{
    HRESULT hr = S_OK;
    TCHAR szGroupName[STRING_NORMAL];
    m_oStr->StringCopy(szGroupName, _T("Favorites"));
    ...
    for(int i = 0; i < MAX_CONST_Favs; i++)
    {
        ...
        sNewGroup->szName       = GetFavoriteName(i);
        sNewGroup->szConstant   = GetFavoriteConstant(i);
        sNewGroup->bIsGroup     = FALSE;
        sNewGroup->iStartIndex  = i;
        sNewGroup->iEndIndex    = i;
        m_arrConstant[1].AddElement(sNewGroup);
    }

    m_iLastIndex = GetGroupCount(0);
...
HRESULT CObjConstants::PopulateGroup(int iLevel, int iPreviousGroupIndex)
{
    ...
    if(iLevel > 0)
    {
        ...
        //good ol favorites
        if(iLevel == 1 && iPreviousGroupIndex == GetGroupCount(0))
            return PopulateGroupFavs();
...
int CObjConstants::GetGroupCount(int iLevel)
{
    ...
    else if(IsFavorite(iLevel))//we're on favorites
    {
        return m_arrFavorites.GetSize();
    }
...
TCHAR* CObjConstants::GetGroupName(int iLevel, int iIndex)
{
    if(iLevel < 0 || iLevel >= MAX_Stars)
        return NULL;
    if(IsFavorite(iLevel))
        return GetFavoriteName(iIndex);
...
TCHAR* CObjConstants::GetGroupConstant(int iLevel, int iIndex)
{
    if(iLevel < 0 || iLevel >= MAX_Stars)
        return NULL;
    if(IsFavorite(iLevel))
        return GetFavoriteConstant(iIndex);
...
TCHAR* CObjConstants::GetSelectedSection(int iLevel)
{
    if(iLevel < 0 || iLevel > 2)
        return NULL;
    if(iLevel == 0 && m_iLastIndex == GetGroupCount(0))
        return m_szFavorites;
...
BOOL CObjConstants::IsFavorite(int iLevel)
{
//    if(iLevel == 1 && m_iLastIndex == GetGroupCount(0))
//        return TRUE;
    return FALSE;
}