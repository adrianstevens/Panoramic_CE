							  ...
							  LPCTSTR pszInstallDir)
{
	...
	{
		MessageBox(hwndParent, _T("Panoramic Call Genius will only work on a Windows Mobile phone edition device.  Please try installing on a proper phone device."), _T("Error"), MB_OK);
		return codeINSTALL_INIT_CANCEL;
	}

    if(IsAppInstalled(_T("Panoramic Phone Genius")))
    {
        MessageBox(hwndParent, _T("Panoramic Phone Genius is currently installed on the device.  Please uninstall Phone Genius before installing Panoramic Call Genius."), _T("Error"), MB_OK);
        return codeINSTALL_INIT_CANCEL;
    }
