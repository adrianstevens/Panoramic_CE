...
// Other Methods
void AX :: Init(TCHAR* cls)
	{
   CLSIDFromString(cls,&clsid);
...
LRESULT CALLBACK AXWndProc(HWND hh,UINT mm,WPARAM ww,LPARAM ll)
   ...
   if (mm == WM_CREATE)
      ...
      ax->Site.Window = hh;
		...
		REFIID rid = *ax->iid;
      hr = OleCreate(ax->GetCLSID(),rid,OLERENDER_NONE,0,&ax->Site,ax->Storage,(void**)&ax->OleObject);
...
// Registration function
    ATOM AXRegister()
    {
        ...
        wc.lpfnWndProc		= (WNDPROC) AXWndProc;
        wc.cbClsExtra		= 0;
        wc.cbWndExtra		= 4;
