...
void CDlgChooseContact::DeleteAllContent()
{
	if(m_oMenu)
		delete m_oMenu;
	m_oMenu = NULL;
...
BOOL CDlgChooseContact::OnDraw(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    ...
    {
        DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_NO_Contacts), m_hInst), rcClient, DT_CENTER|DT_VCENTER, g_gui->GetFontTitle(), RGB(255,255,255), 0);
    }
    else
    {
...
void CDlgChooseContact::DrawSearch(CIssGDIEx& gdi, RECT& rcDraw)
{
    ...
    DrawTextShadow(gdi, m_szSearch, rcDraw, DT_CENTER|DT_VCENTER, g_gui->GetFontSearch(), RGB(150,150,150), 0);
}
...
void CDlgChooseContact::DrawTopBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    RECT rc = m_rcTitle;
    rc.left += INDENT;
    DrawText(gdi, m_oStr->GetText(ID(IDS_TITLE_SelectContact), m_hInst), rc, DT_LEFT | DT_VCENTER, g_gui->GetFontTitle(), RGB(51,51,51));
}
...
void CDlgChooseContact::DrawBottomBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    ...
    DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_MENU_Ok), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontTitle(), RGB(255,255,255), RGB(0,0,0));
        
    rc = m_rcBottomBar;
    rc.left = m_rcBottomBar.right/2;
    DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_MENU_Cancel), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontTitle(), RGB(255,255,255), RGB(0,0,0));
}
