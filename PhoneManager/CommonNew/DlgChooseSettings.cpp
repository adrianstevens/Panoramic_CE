...
void CDlgChooseSettings::DrawTopBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    RECT rc = m_rcTitle;
    rc.left += INDENT;
    DrawText(gdi, m_oStr->GetText(ID(IDS_SELECT_Setting), m_hInst), rc, DT_LEFT | DT_VCENTER, g_gui->GetFontLetters(), RGB(51,51,51));
}
...
void CDlgChooseSettings::DrawListItem(CIssGDIEx& gdi, RECT& rcDraw, BOOL bIsHighlighted, TypeLaunchItem* sItem)
{
    ...
    DrawTextShadow(gdi, sItem->szFriendlyName, rcDraw, DT_LEFT|DT_VCENTER|DT_NOPREFIX, g_gui->GetFontLetters(), RGB(255,255,255), RGB(0,0,0));
}
