...
BOOL CDlgChoosePrograms::OnDraw(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    ...
    if(m_oMenu->GetItemCount() == 0)
    {
        DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_NO_Programs), m_hInst), rcClient, DT_CENTER|DT_VCENTER, g_gui->GetFontLetters(), RGB(255,255,255), 0);
    }
    else
    {
...
void CDlgChoosePrograms::DrawSearch(CIssGDIEx& gdi, RECT& rcDraw)
{
    ...
    DrawTextShadow(gdi, m_szSearch, m_rcArea, DT_CENTER|DT_VCENTER, g_gui->GetFontSearch(), RGB(150,150,150), 0);
}
...
BOOL CDlgChoosePrograms::OnSize(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    if(!CDlgBase::OnSize(hWnd, wParam, lParam))
		return FALSE;
...
void CDlgChoosePrograms::DrawTopBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    RECT rc = m_rcTitle;
    rc.left += INDENT;
    DrawText(gdi, m_oStr->GetText(ID(IDS_SELECT_Program), m_hInst), rc, DT_LEFT | DT_VCENTER, g_gui->GetFontLetters(), RGB(51,51,51));
}
...
void CDlgChoosePrograms::DrawBottomBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    ...
    DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_MENU_Ok), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontLetters(), RGB(255,255,255), RGB(0,0,0));
        
    rc = m_rcBottomBar;
    rc.left = m_rcBottomBar.right/2;
    DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_MENU_Cancel), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontLetters(), RGB(255,255,255), RGB(0,0,0));
}
...
void CDlgChoosePrograms::PopulateList()
{
    ...
    if(m_bAddTodayIcon)
    {
        ...
        { 
			...
			m_oMenu->Move(m_oMenu->GetItemCount()-1, 0);
        }
    }
...
void CDlgChoosePrograms::DrawListItem(CIssGDIEx& gdi, RECT& rcDraw, BOOL bIsHighlighted, TypeLaunchItem* sItem)
{
    ...
    DrawTextShadow(gdi, sItem->szFriendlyName, rcDraw, DT_LEFT|DT_VCENTER|DT_NOPREFIX|DT_END_ELLIPSIS, g_gui->GetFontLetters(), RGB(255,255,255), RGB(0,0,0));
}
