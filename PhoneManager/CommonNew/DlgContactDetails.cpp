...
void CDlgContactDetails::PreloadImages(HWND hWnd, HINSTANCE hInst)
{
    if(!m_imgMask.IsLoaded())
    {
        m_imgMask.Initialize(hWnd, hInst, IsVGA()?IDR_DETAILS_Mask_VGA:IDR_DETAILS_Mask);
    }
    if(!m_imgBorder.IsLoaded())
        m_imgBorder.Initialize(hWnd, hInst, IsVGA()?IDR_DETAILS_Border_VGA:IDR_DETAILS_Border);

    if(m_gdiBackArrow.GetDC() == NULL)
		m_gdiBackArrow.LoadImage(IsVGA()?IDR_DETAILS_BackArrow_VGA:IDR_DETAILS_BackArrow, hWnd, hInst, TRUE);

    // i have to load the default pic here too
    if(m_gdiDefaultPic.GetDC() == NULL)
    {
        CIssGDIEx gdiTemp;
        gdiTemp.LoadImage(IsVGA()?IDR_DETAILS_Default_VGA:IDR_DETAILS_Default, hWnd, hInst, TRUE);
        SIZE sz;
...
void CDlgContactDetails::DrawTopBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    ...
    RECT rc = m_rcTitle;
    rc.left += INDENT;
	DrawText(gdi, szName, rc, DT_LEFT | DT_VCENTER|DT_NOPREFIX, g_gui->GetFontTitle(), RGB(51,51,51));
...
BOOL CDlgContactDetails::DrawButtonItem(CIssGDIEx& gdi, BOOL bIsHighlighted, RECT& rc, EnumContactInfo* eInfo)
{
	if(bIsHighlighted)
    {
        ...
        {
            m_imgButtonSel.Initialize(m_hWnd, m_hInst, g_gui->GetSkin().uiDetailsSelector);
    ...
    else
    {
        ...
        {
			m_imgButton.Initialize(m_hWnd, m_hInst, IsVGA()?IDR_DETAILS_Button_VGA:IDR_DETAILS_Button);
	...
	case CI_MobileNumber:
		m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_Mobile), m_hInst));
		m_oStr->StringCopy(szInfo, m_sContact.szMobileNumber);
		break;
	case CI_HomeNumber:
		m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_Home), m_hInst));
		m_oStr->StringCopy(szInfo, m_sContact.szHomeNumber);
		break;
    case CI_HomeNumber2:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_Home), m_hInst));
		m_oStr->Concatenate(szTitle, _T("2"));
        m_oStr->StringCopy(szInfo, m_sContact.szHomeNumber2);
        break;
    case CI_HomeFax:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_HomeFax), m_hInst));
	...
	case CI_SMS:
		...
		break;
	case CI_WorkNumber:
		m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_Work), m_hInst));
    ...
    case CI_WorkNumber2:
        ...
        break;
    case CI_WorkFax:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_WorkFax), m_hInst));
        m_oStr->StringCopy(szInfo, m_sContact.szWorkFax);
        break;
    case CI_Car:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_Car), m_hInst));
        m_oStr->StringCopy(szInfo, m_sContact.szCar);
        break;
    case CI_Pager:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_Pager), m_hInst));
        m_oStr->StringCopy(szInfo, m_sContact.szPager);
        break;

	case CI_Email:
		m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_Email), m_hInst));
    ...
    case CI_Email3:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_ITEM_Email), m_hInst));
		m_oStr->Concatenate(szTitle, _T("3"));
	...
	DrawTextShadow(gdi, szTitle, rcText, DT_LEFT | DT_VCENTER | DT_END_ELLIPSIS, g_gui->GetFontTitle(), RGB(255,255,255), RGB(0,0,0));

    // value text
	rcText.top		+= HEIGHT(rc)/2;
	rcText.bottom	+= HEIGHT(rc)/2;
    DrawText(gdi, szInfo, rcText, DT_LEFT | DT_TOP| DT_END_ELLIPSIS, g_gui->GetFontText(), (bIsHighlighted?RGB(0,0,0):RGB(167,167,167)));
...
void CDlgContactDetails::DrawBottomBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    ...
    {
        DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_Back), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontTitle(), RGB(255,255,255), RGB(0,0,0));
    ...
    DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_MENU_Menu), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontTitle(), RGB(255,255,255), RGB(0,0,0));*/
