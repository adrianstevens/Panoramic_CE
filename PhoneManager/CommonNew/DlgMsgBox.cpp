...
void CDlgMsgBox::DrawTopBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
	...
	rc.left += INDENT;
    if(m_szTitle)
	    ::DrawText(gdi, m_szTitle, rc, DT_LEFT | DT_VCENTER, g_gui->GetFontTitle(), RGB(51,51,51));
}
...
void CDlgMsgBox::DrawBottomBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    ...
    if(m_bYesNo)
        DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_Yes), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontTitle(), RGB(255,255,255), RGB(0,0,0));
    else
    ...
    if(m_bYesNo)
        DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_No), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontTitle(), RGB(255,255,255), RGB(0,0,0));
}
