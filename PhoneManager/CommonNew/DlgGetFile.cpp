...
BOOL CDlgGetFile::OnInitDialog(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	if(m_bOpenFile)
        SetWindowText(m_hWnd, m_oStr->GetText(ID(IDS_OPEN_File), m_hInst));
	else
        SetWindowText(m_hWnd, m_oStr->GetText(ID(IDS_SAVE_File), m_hInst));
...
void CDlgGetFile::OnMenuOpen()
{
	...
	{
		MessageBox(m_hWnd, _T("File does not exist"), _T("Error"), MB_OK);
		return;
	}
...
{
    ::DrawText(hDC, m_oStr->GetText(ID(IDS_Name), m_hInst), m_rcName, DT_LEFT|DT_VCENTER, m_hFontText, RGB(0,0,0));
    ::DrawText(hDC, m_oStr->GetText(ID(IDS_Type), m_hInst), m_rcType, DT_LEFT|DT_VCENTER, m_hFontText, RGB(0,0,0));
...
void CDlgGetFile::SaveFile(TCHAR* szFileName)
{
	...
	else if(dwAttributes != ((DWORD)-1))
	{
        ...
        if(IDYES != MessageBox(m_hWnd, m_oStr->GetText(ID(IDS_FILE_ExistOverwrite), m_hInst), m_oStr->GetText(IDS_Warning, m_hInst), MB_YESNO))
			return;
	}
...
BOOL CCtrlBrowse::GetCurrentFolderName(TCHAR* szFolderName)
{
	...
	if(IsRootPath())
	{
        m_oStr->StringCopy(szFolderName, m_oStr->GetText(IDS_MY_Device, m_hInst));
		return TRUE;
	}
...
void CCtrlBrowse::OnSelectItem(BOOL bDoubleTap)
{
	LVITEM lvi = {0};
	lvi.mask		= LVIF_PARAM;
	lvi.iItem = ListView_GetNextItem(m_hWnd, -1, LVNI_SELECTED);
	...
	if(dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY && bDoubleTap)
	{
