...
BOOL CGuiContacts::Draw(HDC hDC, RECT& rcClient, RECT& rcClip)
{
    ...
    if(!GetItemCount())
        DrawText(m_gdiBg->GetDC(), _T("no entries"), m_rcList, DT_CENTER , 0, m_crText);
    else
...
void CGuiContacts::DrawListItem(CIssGDIEx& gdi, RECT& rcDraw, TypeItems* sItem, BOOL bIsHighlighted)
{
    ...
    if(bIsHighlighted)
    {
        ::DrawText(gdi.GetDC(), szDrawText, 
            rcDraw.left + iIndent, rcDraw.top, DT_LEFT|DT_NOPREFIX, m_hFontContactBold, COLOR_TEXT_NORMAL);
        RECT rcLine2 = rcDraw;
    ...
    {
        ::DrawText(gdi.GetDC(), szDrawText, 
            rcDraw.left + iIndent, rcDraw.top, DT_LEFT|DT_NOPREFIX, 0, COLOR_TEXT_NORMAL);
    }
}
...
HRESULT CGuiContacts::PoomItemAdded(long lOid)
{
    ...
    CPHR(sContact, _T("sContact is NULL"));
...
{
	CIssString* oStr = CIssString::Instance();
    TCHAR szCompare1[STRING_MAX] = {0};
	TCHAR szCompare2[STRING_MAX] = {0};
...
{
    if(GetEndOfListYPos() < GetOwnerFrame().bottom)
        return;
