...
{
    s_guiToolbar = this;
	m_hFontBtnText = CIssGDIEx::CreateFont(13*GetSystemMetrics(SM_CXICON)/32, FW_NORMAL, TRUE);
}
...
BOOL CGuiToolBar::Init(HWND hWndParent, HINSTANCE hInst, CIssGDIEx* gdiMem, CIssGDIEx* gdiBg, CGuiBackground* guiBackground, TypeOptions* sOptions, CDlgLauncher* dlgParent)
{
    CGuiBase::Init(hWndParent, hInst, gdiMem, gdiBg, guiBackground, sOptions);
    m_dlgParent = dlgParent;

    m_gdiImgIconArray.LoadImage(IsVGA()?IDR_PNG_ToolbarIconArrayVGA:IDR_PNG_ToolbarIconArray, hWndParent, hInst, TRUE);
...
void CGuiToolBar::ReloadTextItems()
{
    m_oStr->StringCopy(m_szBtnText[0], ID(IDS_TOOLBAR_FAV),         STRING_NORMAL, m_hInst);
    m_oStr->StringCopy(m_szBtnText[1], ID(IDS_TOOLBAR_PROGRAMS),    STRING_NORMAL, m_hInst);
    m_oStr->StringCopy(m_szBtnText[2], ID(IDS_TOOLBAR_SETTINGS),     STRING_NORMAL, m_hInst);
    //m_oStr->Empty(m_szBtnText[3]);
    m_oStr->StringCopy(m_szBtnText[3], ID(IDS_Back),        STRING_NORMAL, m_hInst);
...
BOOL CGuiToolBar::OnMouseMove(POINT& pt)
{
    ...
    hr = gdiTemp.Create(hdcScreen, rcDraw, FALSE, TRUE, FALSE);
    FillRect(gdiTemp, rcDraw, 0);
