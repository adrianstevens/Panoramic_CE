...
void CDlgAbout::DrawTopBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    RECT rc = m_rcTitle;
    rc.left += INDENT;
    ::DrawText(gdi, m_oStr->GetText(ID(IDS_About_Launchpad), m_hInst), rc, DT_LEFT | DT_VCENTER, g_gui->GetFontTitle(), RGB(51,51,51));
}
...
void CDlgAbout::DrawBottomBar(CIssGDIEx& gdi, RECT& rcClient, RECT& rcClip)
{
    ...
    DrawTextShadow(gdi, m_oStr->GetText(ID(IDS_Close), m_hInst), rc, DT_CENTER|DT_VCENTER, g_gui->GetFontTitle(), RGB(255,255,255), RGB(0,0,0));
}
