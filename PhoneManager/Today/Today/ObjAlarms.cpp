...
BOOL CObjAlarms::GetFormattedAlarmTime(int iAlarm, TCHAR* szAlarm)
{
    AlarmType* sAlarm = GetAlarm(iAlarm);
    if(sAlarm == NULL)
        return FALSE;

    //now format the time using AM/PM
    int iHours = sAlarm->dwHours;
    BOOL bPM = FALSE;
    if(iHours > 12)
    {
        iHours -= 12;
    ...
    return m_oStr->Format(szAlarm, _T("%d:%02d %s"), iHours, sAlarm->dwMinutes, (bPM?_T("PM"):_T("AM")));
}
