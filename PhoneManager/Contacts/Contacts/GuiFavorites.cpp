...
BOOL CGuiFavorites::AddMenuItems()
{
    ...
    m_wndMenu->AddItem(IDS_MENU_SetContactPicture, m_hInst, IDMENU_SetContactPicture, iCount&&pContact?NULL:FLAG_Grayed);
    m_wndMenu->AddItem(IDS_MENU_SetContactRingtone, m_hInst, IDMENU_SetContactRingtone, iCount&&pContact?NULL:FLAG_Grayed);
    m_wndMenu->AddItem(IDS_MENU_SetContactCamera, m_hInst, IDMENU_SetContactCamera, iCount&&pContact?NULL:FLAG_Grayed);
...
BOOL CGuiFavorites::MoveGui(RECT rcLocation)
{
  ...
  /*  switch(m_sOptions->eFavSize)
    {
    case FAVSIZE_Large:
        m_hFont = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXICON)*3/7 + 3*(1+IsVGA()), FW_BOLD, TRUE);
    	break;
    case FAVSIZE_Normal:
        m_hFont = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXICON)*3/7, FW_BOLD, TRUE);
    	break;
    case FAVSIZE_Small:
        m_hFont = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXICON)*3/7 - 2*(1+IsVGA()), FW_NORMAL, TRUE);
        break;
    default:
        break;
    }*/
    ...
    {
    case FAVSIZE_Large:
        m_hFont = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXICON)*14/32, FW_NORMAL, TRUE);
        break;
    case FAVSIZE_Normal:
        m_hFont = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXICON)*13/32, FW_NORMAL, TRUE);
        break;
    case FAVSIZE_Small:
        m_hFont = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXICON)*12/32, FW_NORMAL, TRUE);
        break;
    default:
        break;
    }
...
BOOL CGuiFavorites::Draw(CIssGDIEx& gdiMem, HDC hDC, RECT& rcClient, RECT& rcClip)
{
...
void CGuiFavorites::SlidePage()
{
    ...
    while(i < iNumFrames)
    {
        ...
        if(guiToolbar)
            guiToolbar->DrawSelector(*m_gdiBg, m_rcGrid, m_rcGrid, FALSE);
...
CIssGDIEx&  CGuiFavorites::GetMovingNameImage()
{
    ...
    if(sItem)
    {
       ...
       if(sContactInfo && pPoom)
       {
           pPoom->GetFormatedNameText(szName, sContactInfo->szFirstName, sContactInfo->szMiddleName, sContactInfo->szLastName, sContactInfo->szCompany, NULL, TRUE);
...
BOOL CGuiFavorites::OnKeyUp(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    if(wParam == VK_TTALK)
    {
        ...
        */
        ...
        // 1-13-2009
        // message box: ask if user wants to call selected contact
...
BOOL CGuiFavorites::OnSliderButtonUp(POINT pt)
{
    ...
    if(m_bSliderDrag)
    {
		...
		// make sure the choose contact dialog is reset so it reloadsm_
		CDlgChooseContact::ResetListContent();
...
void CGuiFavorites::AnimateSlider(POINT pt, int iDestPage)
{
    ...
    DWORD dwAnimateTime = FAVORITES_SLIDER_ANIMATE_TIME;
    if(m_sOptions->bShowAnimations == FALSE)
        dwAnimateTime = 1;
    ...
    float fAnimateTime = 100.0f + (float)dwAnimateTime * iNumFrames / iMaxTravel;
...
BOOL CGuiFavorites::OnLButtonDown(POINT pt)
{
    ...
    if(m_iHoldingIndex != UND)
	{
		...
		sItem->DrawItem(m_gdiMovingItem.GetDC(), rcItem, GetPlaceHolderImg(), m_hFont, TRUE, NULL, m_sOptions->bAlwaysShowText);
    ...
    case IDMENU_SetContactPicture:
        {
            ...
            if(pContact)
            {
                ...
                CPoomContacts::Instance()->SetContactPicture(pContact->GetOid());
            }
        }
        break;
	...
	case IDMENU_EditContact:
        {
            ...
            if(pContact)
            {
                // give back taskbar so user can close the outlook window
                SHFullScreen(m_hWndParent, SHFS_SHOWTASKBAR | SHFS_SHOWSIPBUTTON);
                CPoomContacts::Instance()->LaunchEditContactScreen(m_hWndParent, pContact->GetOid());
            }
       }
		break;
	case IDMENU_NewContact:
        {
            // give back taskbar so user can close the outlook window
		    SHFullScreen(m_hWndParent, SHFS_SHOWTASKBAR | SHFS_SHOWSIPBUTTON);
            CPoomContacts::Instance()->CreateNew(m_hWndParent);
        }
		break;
	...
	for(int i = 0; i < arrContacts.GetSize(); i++)
	{
		...
		if(sItem /*&& i != iExcludeItem*/)
		{
			...
			sItem->DrawItem(gdiTemp.GetDC(), rcItem/*rcClient*/, GetPlaceHolderImg(), 
                m_hFont, TRUE, NULL, m_sOptions->bAlwaysShowText);
...
void CGuiFavorites::AddItem()
{
	...
	CDlgChooseContact dlgAdd(m_hWndParent, &GetCurContactArray(), bLastNameFirst);
...
void CGuiFavorites::SetIconSize(EnumFavSize eFavSize)
{
    ...
    MoveGui(rc);
    AutoArrangeItems(m_iPersonalMatrix, m_arrPersonal);
    AutoArrangeItems(m_iBusinessMatrix, m_arrBusiness);
}
...
void CGuiFavorites::AnimateIn(CIssGDIEx& gdiDest, RECT& rcClient, int iAnimeStep, int iAnimTotal)
{
	...
	for(int i=0; i<GetCurContactArray().GetSize(); i++)
	{
		...
		if(sItem)
		{
            ...
            sItem->DrawItem(gdiDest.GetDC(), rcClient, GetPlaceHolderImg(), m_hFont, FALSE, &ptCurrent, m_sOptions->bAlwaysShowText);
		}
	}
}
...
void CGuiFavorites::SetContactCamera()
{
    CObjItemContact* pContact = GetCurContactArray()[GetSelected()];
    if(!pContact)
        return;
    ...
    TCHAR* szTitle = pPoom->GetUserData(CI_FileAs, pContact->GetOid());
    if(!szTitle)
    {
    ...
    {
        pPoom->AddPictureToContact(pContact->GetOid(), shcc.szFile);
        DeleteFile(shcc.szFile);
    }

}

void CGuiFavorites::SetContactRingtone()
{
    CObjItemContact* pContact = GetCurContactArray()[GetSelected()];
    if(!pContact)
        return;
    ...
    int iIndex = pPoom->GetIndexFromOID(pContact->GetOid());

    CDlgContactRingtone dlgRingtone;

    dlgRingtone.SetIndex(iIndex);
    dlgRingtone.Init(m_gdiMem, m_guiBackground, TRUE, TRUE);
    dlgRingtone.Launch(m_hWndParent, m_hInst, FALSE);
}
