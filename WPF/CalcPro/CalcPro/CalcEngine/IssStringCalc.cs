...
namespace CalcPro
{
    public class IssStringCalc
    {
        ...
        virtual public void SaveState(string name)
        {
          ...
          /*      using (System.IO.IsolatedStorage.IsolatedStorageFileStream fs = isf.CreateFile(name))
                {
                    //and serialize data
        ...
        virtual public void RestoreState(string name)
        {
           ...
           /*     if (isf.FileExists(name))
                {
                    //If user choose to save, create a new file
                    using (System.IO.IsolatedStorage.IsolatedStorageFileStream fs = isf.OpenFile((name), System.IO.FileMode.Open))
                    {
                        //and serialize data
                        System.Xml.Serialization.XmlSerializer ser = new System.Xml.Serialization.XmlSerializer(typeof(StringState));
                        sState = (StringState)ser.Deserialize(fs);
                    }
