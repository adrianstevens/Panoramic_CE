...
BOOL CDlgMsgBox::OnSize(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	CIssGDIEx::DeleteFont(m_hFontText);
	CIssGDIEx::DeleteFont(m_hFontSelected);
	m_hFontText				= CIssGDIEx::CreateFont(GetSysMets(SM_CXICON)*3/6, FW_BOLD, TRUE);
	m_hFontSelected			= CIssGDIEx::CreateFont(GetSysMets(SM_CXICON)*5/9, FW_BOLD, TRUE);

	RECT rcClient;
	GetClientRect(m_hWnd, &rcClient);
	int iShadowSpace	= 2*HEIGHT(rcClient)/100;
	int iBtnWidth		= GetSysMets(SM_CXSCREEN)/4;
	int iBtnHeight		= GetSysMets(SM_CYICON);
	int iTextHeight		= GetSysMets(SM_CXICON)/2;
	int iTotalHeight	= 0;
	...
	m_rcText			= rcClient;
	m_rcText.top		= iYpos;
	m_rcText.bottom		= iYpos + 8*iTextHeight;
...
BOOL CDlgMsgBox::FadeInScreen(HDC hDC, RECT rcClient)
{
	...
	int iWidth	= GetSysMets(SM_CXSCREEN);
	int iHeight = GetSysMets(SM_CYSCREEN);
