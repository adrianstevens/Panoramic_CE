...
//
//
// CChords can be built like: | = seperate fields, [ ... ] = range of values
//                           <> = nothing
//
//                          [<--optional--->]
//                          [<--repeating-->]
...
#define POSITION_SCORE iScorePos//2
...
// here:
//so for bass strings we can just null out the last for 5 string and last 2 for 4 string
...
//
void CChord::print(char *cname)
{
	printf("CChord %s : ", cname);
	for (int i = 0; i < 12; i++)
	{
		if (notes[i] != -1)
...
// array of the chord
//
int CChord::covered(int *noteArray)
{
...
void Fretboard::SetScoreVars(bool bBase)
{
	if(bBase)
	{
		iScorePos			= 10;
...
//
void Fretboard::getScore(CChord &chord)
{
    ...
    for(int i = 0; i < getNumStrings(); i++)
    {
        ...
        {
            iCount++;
            score[SCORE_NUMNOTES] += NUMSTRINGS_SCORE;
        }
    }

    //penalized
    if(iCount < 3)
        score[SCORE_NUMNOTES] = LOW_SCOUNT_PENALTY;
...
//
void Fretboard::iterate(CChord &chord)
{
	int string = 0;
	...
	while (string < getNumStrings())
	{
		...
		while (! chord.inCChord(notes[string]))
		{
			fretboard[string]++;
			notes[string] = ( notes[string] + 1 ) % 12;
		}
...
int main()
{
	...
	while (gets(buf) != NULL)
	{
		...
		// findCChord upppercases the input string, so save a copy
		strcpy(buf2, buf);
...
void Fretboard::SetOpenStrings(int i1, int i2, int i3, int i4, int i5, int i6, int iStringCount)
{
	openStrings[0] = i1;
	openStrings[1] = i2;
	openStrings[2] = i3;
