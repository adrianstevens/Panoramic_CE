...
,m_bPlaySounds(2)
{
    SetRect(&m_rcBottomBar, 0, GetSysMets(SM_CYSCREEN) - MENU_HEIGHT, GetSysMets(SM_CXSCREEN), GetSysMets(SM_CYSCREEN));
    SetRect(&m_rcButtons, 0, 0, GetSysMets(SM_CXSCREEN), BUTTONS_HEIGHT);
    SetRect(&m_rcNut, 0, BUTTONS_HEIGHT, GetSysMets(SM_CXSCREEN), BUTTONS_HEIGHT + NUT_HEIGHT);
    ...
    for(int i = 0; i < 6; i++)
    {
        ...
        if(GetSysMets(SM_CXSCREEN)%240 != 0)
        {   //this would probably work for everything
            iIndent = 78*GetSysMets(SM_CXSCREEN)/480;
            iSpacing = 64*GetSysMets(SM_CXSCREEN)/480;
        }

        SetRect(&m_rcStrings[i], iIndent+i*iSpacing, BUTTONS_HEIGHT, 
            iIndent+i*iSpacing + STRING_WIDTH, GetSysMets(SM_CYSCREEN) - MENU_HEIGHT);
...
BOOL CObjGui::DrawNut(RECT& rcClip, CIssGDIEx* gdi /* = NULL */)
{
    ...
    {
        if(GetSysMets(SM_CXSCREEN)%240 != 0)//we need to scale
        {
            CIssGDIEx gdiTemp;
...
BOOL CObjGui::DrawStrings(RECT& rcClip, int iFade, CIssGDIEx* gdi /* = NULL */)
{
    ...
    {
        if(GetSysMets(SM_CYSCREEN) > 400 || IsVGA())
        {
...
BOOL CObjGui::DrawMenu(RECT& rcClip, CIssGDIEx* gdi /* = NULL */, BOOL bDrawIcons /* TRUE */ )
{
    if(m_imgMenu.GetImage().GetDC() == NULL)
    {
        ...
        m_imgMenu.SetSize(GetSysMets(SM_CXSCREEN), HEIGHT(m_rcBottomBar));
    }
...
BOOL CObjGui::DrawTopButtons(RECT& rcClip, CIssGDIEx* gdi /* = NULL */)
{
    if(m_imgButtons.GetImage().GetDC() == NULL)
    {
        ...
        m_imgButtons.SetSize(GetSysMets(SM_CXSCREEN), HEIGHT(m_rcButtons));
    }
...
void CObjGui::InitBackground(RECT& rcClient)
{
    ...
    m_rcBackground.right	= GetSysMets(SM_CXSCREEN);
    m_rcBackground.bottom	= GetSysMets(SM_CYSCREEN);
...
/*    m_eMenuSelect = MENU_None;
    if(PtInRect(&m_rcBottomBar, pt))
    {        
        if(pt.x < GetSysMets(SM_CXSCREEN)/2)
...
CIssGDIEx* CObjGui::GetGDI(BOOL bDestroy /* = FALSE */)
{
    ...
    {
        HDC dc = GetDC(m_hWndParent);
        m_gdiMem.Create(dc, GetSysMets(SM_CXSCREEN), GetSysMets(SM_CYSCREEN));
        ReleaseDC(m_hWndParent, dc);
    }    
    return &m_gdiMem;
}
...
HFONT CObjGui::GetFont(EnumFont eFont)
{
    ...
    if(m_hFonts[eFont] == NULL)
    {
        ...
        case FONT_Button://intentionally not exact scale
            ...
            if(GetSysMets(SM_CXSCREEN) == 320 && GetSysMets(SM_CYSCREEN) == 320)
        ...
        case FONT_Menu://just this one is scaled
            m_hFonts[eFont] = CIssGDIEx::CreateFont(GetSysMets(SM_CXICON)*11/32, FW_BOLD, TRUE);
            break;
        case FONT_MenuSmall://just this one is scaled
            m_hFonts[eFont] = CIssGDIEx::CreateFont(GetSysMets(SM_CXICON)*11/32, FW_NORMAL, TRUE);
            break;
        case FONT_MenuLarge://just this one is scaled
            m_hFonts[eFont] = CIssGDIEx::CreateFont(GetSysMets(SM_CXICON)*14/32, FW_BOLD, TRUE);
            break;
        case FONT_Fingers:
...
/*	if(m_oSoundFX.IsLoaded() == FALSE)
	...
	//works
	BOOL bRet = PlaySound(
		MAKEINTRESOURCE(iIndex + LOWEST_SOUND_RES), 
