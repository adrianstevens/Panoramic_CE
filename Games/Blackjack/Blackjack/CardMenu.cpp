...
#endif
{
    ...
    m_hFontSm = CIssGDIEx::CreateFont(GetSystemMetrics(SM_CXICON)*2/5, 700, TRUE);
...
BOOL CCardMenu::DrawMenu(CIssGDIEx& gdi, RECT& rc, CIssBJGame* oGame, HINSTANCE hInst)
{
    HFONT hFontTemp = m_hFont;
    ...
    case GSTATE_SplitMove:
        bDrawLines = TRUE;
        {
            ...
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Hit), hInst), m_rcTopText[MENU_Hit], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
            //m_fntWhite->DrawText(gdi, _T("Hit"), m_rcTopText[MENU_Hit], DT_CENTER | DT_VCENTER);
            
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Stand), hInst), m_rcTopText[MENU_Stand], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
            //m_fntWhite->DrawText(gdi, _T("Stand"), m_rcTopText[MENU_Stand], DT_CENTER | DT_VCENTER);
            
            if(oGame->ShowDoubleButton())
                DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Double), hInst), m_rcTopText[MENU_Double], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
                //m_fntWhite->DrawText(gdi, _T("Double"), m_rcTopText[MENU_Double], DT_CENTER | DT_VCENTER);
            else
                DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Double), hInst), m_rcTopText[MENU_Double], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
                //m_fntGray->DrawText(gdi, _T("Double"), m_rcTopText[MENU_Double], DT_CENTER | DT_VCENTER);

            if(oGame->ShowSplitButton())
                DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Split), hInst), m_rcTopText[MENU_Split], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
                //m_fntWhite->DrawText(gdi, _T("Split"), m_rcTopText[MENU_Split], DT_CENTER | DT_VCENTER);
            else
                DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Split), hInst), m_rcTopText[MENU_Split], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
    ...
    case GSTATE_Bet:
        bDrawLines = TRUE;
        {
            ...
            if(oGame->Show25())
                DrawText(gdi.GetDC(), _T("$25"), m_rcTopText[MENU_25], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
            else
                DrawText(gdi.GetDC(), _T("$25"), m_rcTopText[MENU_25], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
                //m_fntGreen->DrawText(gdi, _T("$25"), m_rcTopText[MENU_25], DT_CENTER | DT_VCENTER);
            if(oGame->Show50())
                DrawText(gdi.GetDC(), _T("$50"), m_rcTopText[MENU_50], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
            else
                DrawText(gdi.GetDC(), _T("$50"), m_rcTopText[MENU_50], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
                //m_fntGreen->DrawText(gdi, _T("$50"), m_rcTopText[MENU_50], DT_CENTER | DT_VCENTER);
            if(oGame->Show100())
                DrawText(gdi.GetDC(), _T("$100"), m_rcTopText[MENU_100], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
            else
                DrawText(gdi.GetDC(), _T("$100"), m_rcTopText[MENU_100], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
                //m_fntGreen->DrawText(gdi, _T("$100"), m_rcTopText[MENU_100], DT_CENTER | DT_VCENTER);
            if(oGame->Show500())
                DrawText(gdi.GetDC(), _T("$500"), m_rcTopText[MENU_500], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
            else
                DrawText(gdi.GetDC(), _T("$500"), m_rcTopText[MENU_500], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
    ...
    case GSTATE_EndOfGame:
        if(oGame->IsFinishedDealing())
        {
            ...
            {   //two scores yo
                ...
                {
                case GRESULT_Win:
                    m_oStr->StringCopy(szPlayer, m_oStr->GetText(ID(IDS_RESULT_Win), hInst));
                    break;
                case GRESULT_Lose:
                    m_oStr->StringCopy(szPlayer, m_oStr->GetText(ID(IDS_RESULT_Lose), hInst));
                    break;
                case GRESULT_BlackJack:
                    m_oStr->StringCopy(szPlayer, m_oStr->GetText(ID(IDS_RESULT_Blackjack), hInst));
                    break;
                case GRESULT_Surrender:
                    m_oStr->StringCopy(szPlayer, m_oStr->GetText(ID(IDS_RESULT_Surrender), hInst));
                default:
                case GRESULT_Draw:
                    m_oStr->StringCopy(szPlayer, m_oStr->GetText(ID(IDS_RESULT_Draw), hInst));
                    break;
                }
                ...
                {
                case GRESULT_Win:
                    m_oStr->StringCopy(szSplit, m_oStr->GetText(ID(IDS_RESULT_Win), hInst));
                    break;
                case GRESULT_Lose:
                    m_oStr->StringCopy(szSplit, m_oStr->GetText(ID(IDS_RESULT_Lose), hInst));
                    break;
                case GRESULT_BlackJack:
                    m_oStr->StringCopy(szSplit, m_oStr->GetText(ID(IDS_RESULT_Blackjack), hInst));
                    break;
                case GRESULT_Surrender:
                    m_oStr->StringCopy(szSplit, m_oStr->GetText(ID(IDS_RESULT_Surrender), hInst));
                default:
                case GRESULT_Draw:
                    m_oStr->StringCopy(szSplit, m_oStr->GetText(ID(IDS_RESULT_Draw), hInst));
                    break;
                }

                m_oStr->Format(szResults, _T("%s - %s"), szPlayer, szSplit);
                DrawText(gdi.GetDC(), szResults, m_rcMenuAlphaText, DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
           }
            ...
            else
            {
                ...
                case GRESULT_Win:
                    if(oGame->DealerBusts())
                        DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_RESULT_DealerBusts), hInst), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
                    //m_fntGreen->DrawText(gdi, _T("Dealer Busts - Win!"), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER);
                    else
                        DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_RESULT_Win2), hInst), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
                    //m_fntGreen->DrawText(gdi, _T("Win!"), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER);
                    break;
                case GRESULT_Lose:
                    DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_RESULT_Lose), hInst), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
                    //m_fntGreen->DrawText(gdi, _T("Lose"), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER);
                    break;
                case GRESULT_BlackJack:
                    DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_RESULT_Blackjack), hInst), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
                    //m_fntGreen->DrawText(gdi, _T("Blackjack!"), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER);
                    break;
                case GRESULT_Draw:
                    DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_RESULT_Draw), hInst), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
                    //m_fntGreen->DrawText(gdi, _T("Draw"), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER);
                    break;
                case GRESULT_Surrender:
                    DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_RESULT_Surrender), hInst), m_rcMenuAlphaText, DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
    ...
    case GSTATE_Bet:
        bDrawLines = TRUE;
        if(oGame->GetGameState() == GSTATE_Bet)
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Bet), hInst), m_rcBottomText[MENU_Bet], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_GREEN);
        else
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Bet), hInst), m_rcBottomText[MENU_Bet], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
        //m_fntWhite->DrawText(gdi, _T("Bet"), m_rcBottomText[MENU_Bet], DT_CENTER | DT_VCENTER);
        if(oGame->ShowDealButton())
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Deal), hInst), m_rcBottomText[MENU_Deal], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
            //m_fntWhite->DrawText(gdi, _T("Deal"), m_rcBottomText[MENU_Deal], DT_CENTER | DT_VCENTER);
        else
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Deal), hInst), m_rcBottomText[MENU_Deal], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
            //m_fntGray->DrawText(gdi, _T("Deal"), m_rcBottomText[MENU_Deal], DT_CENTER | DT_VCENTER);
        if(oGame->ShowDealButton())
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Reset), hInst), m_rcBottomText[MENU_Reset], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
            //m_fntWhite->DrawText(gdi, _T("Reset"), m_rcBottomText[MENU_Reset], DT_CENTER | DT_VCENTER);
        else
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Reset), hInst), m_rcBottomText[MENU_Reset], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
    ...
    case GSTATE_SplitMove:
        if(oGame->ShowInsuranceButton())
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Insurance), hInst), m_rcBottomLrg[0], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
            //m_fntWhite->DrawText(gdi, _T("Insurance"), m_rcBottomLrg[0], DT_CENTER | DT_VCENTER);
        else if(oGame->IsInsurance())
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Insurance), hInst), m_rcBottomLrg[0], DT_CENTER | DT_VCENTER, hFontTemp,  RGB(255, 239,0));
        else
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Insurance), hInst), m_rcBottomLrg[0], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
            //m_fntGray->DrawText(gdi, _T("Insurance"), m_rcBottomLrg[0], DT_CENTER | DT_VCENTER);
        if(oGame->ShowSurrenderButton())
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_RESULT_Surrender), hInst), m_rcBottomLrg[1], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
            //m_fntWhite->DrawText(gdi, _T("Surrender"), m_rcBottomLrg[1], DT_CENTER | DT_VCENTER);
        else
            DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_RESULT_Surrender), hInst), m_rcBottomLrg[1], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
            //m_fntGray->DrawText(gdi, _T("Surrender"), m_rcBottomLrg[1], DT_CENTER | DT_VCENTER);
        break;
    case GSTATE_DealerMove:
    case GSTATE_Deal:
        bDrawLines = TRUE;
        DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_BTNS_Bet), hInst), m_rcBottomText[MENU_Bet], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_DIMMED);
    ...
    DrawText(gdi.GetDC(), m_oStr->GetText(ID(IDS_MENU_Menu), hInst), m_rcBottomText[MENU_Menu], DT_CENTER | DT_VCENTER, hFontTemp, TEXT_COLOR);
