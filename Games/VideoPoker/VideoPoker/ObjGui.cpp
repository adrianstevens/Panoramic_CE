...
void CObjGui::InitBackground(RECT& rcClient)
{
    ...
    if(gdiBackground.GetDC() == NULL)
    {
        ...
        gdiTemp.LoadImage(IsVGA()?IDB_PNG_Logo_HR:IDB_PNG_Logo, m_hWndParent, m_hInst);

        POINT pt;
        pt.x = (gdiBackground.GetWidth() - gdiTemp.GetWidth())/2;
        pt.y = 0; //(gdiBackground.GetHeight())/2 + (gdiBackground.GetHeight()/2 - gdiTemp.GetHeight())/2;
