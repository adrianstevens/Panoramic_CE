...
BOOL CDlgSplashScreen::OnPaint(HWND hWnd, HDC hDC, RECT& rcClient)
{
	...
	if(m_iPercent >= 0 && m_iPercent <= 100)
	{
		...
		CIssGDI::DrawGradientRoundRectangle(m_gdiMem, rc, RGB(0,0,0), iCurve, iCurve, RGB(194, 147, 206), RGB(51,26, 57));
...
BOOL CDlgSplashScreen::FadeInScreen(HDC hDC, RECT rcClient)
{
	...
	CIssGDI::MyAlphaImage(m_gdiImage, 
						  (iWidth - m_gdiSplash.GetWidth())/2, (iHeight - m_gdiSplash.GetHeight())/2,
						  m_gdiSplash.GetWidth(), m_gdiSplash.GetHeight(),
						  m_gdiSplash, m_gdiSplashAlpha,
						  0,0);

	int iCurve		= GetSystemMetrics(SM_CXICON)/4;
	int iIndent		= m_gdiSplashAlpha.GetWidth()/10;
	int iBarHeight	= m_gdiSplashAlpha.GetHeight()/20;
	...
	CIssGDI::DrawGradientRoundRectangle(m_gdiImage, m_rcPercent, RGB(0,0,0), iCurve, iCurve, RGB(0,0,0), RGB(100,100,100));

	
	CIssGDIEffects::FadeIn(hDC, m_gdiImage);
...
BOOL CDlgSplashScreen::OnUser(HWND hWnd, UINT uiMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LOAD_Background:
		...
		PostMessage(hWnd, WM_lOAD_Hexagons, 0,0);
		break;
