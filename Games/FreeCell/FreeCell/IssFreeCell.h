...
enum EnumFreecellType
{
    FCTYPE_Freecell,   
    FCTYPE_Bakers, //suits instead of colors
    FCTYPE_Kingcell,
    FCTYPE_FourColors,
    FCTYPE_Count,
};

enum EnumFreeCellCount
{
    FCCOUNT_4,
    FCCOUNT_3,
    FCCOUNT_2,
    FCCOUNT_1,//won't use as its too damn hard
    FCCOUNT_Count,
};
...
#define MAX_CARDS_PER_COLUMN	24 //just in case for four colors mode
...
//	BOOL		bCanUndo;
	...
	byte		btFreeCells[4];
	byte		btAceRacks[4];
//	void		ClearUndo();//really all we need
...
public:
    ...
    EnumFreecellType GetGameType(){return m_eGameType;};
    void        SetGameType(EnumFreecellType eType);

    BOOL        GetInverse(){return m_bInverse;};
    void        SetInverse(BOOL bInverse){m_bInverse = bInverse;};
    BOOL        GetStrict(){return m_bStrict;};
    void        SetStrict(BOOL bStrict){m_bStrict = bStrict;};
    BOOL        GetShowTimer(){return m_bShowTimer;};
    void        SetShowTimer(BOOL bTimer){m_bShowTimer = bTimer;};
...
private:
	...
	BOOL		MoveCardFromColumntoColumn(byte btFrom, byte btTo, BOOL bMakeMove = FALSE, BOOL bMulti = TRUE);
    ...
    BOOL        CanBuild(byte btFrom, byte btTo); //color, suit, etc depending on game .. doesn't check value
...
//	TypeFreeCellUndo m_sUndo;
	CIssVector<TypeFreeCellUndo> m_arrUndo;//should be fine
