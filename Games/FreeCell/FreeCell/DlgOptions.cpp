...
BOOL CDlgOptions::DrawButtonItem(CIssGDIEx& gdi, BOOL bIsHighlighted, RECT& rc, EnumOption* eOption)
{
    ...
    case OPTION_Skin:
        ...
        break;
    case OPTION_AutoComplete:
        GetAutoCompleteText(m_eAutoPlay, szInfo);
        break;
    case OPTION_Strict:
        m_oStr->StringCopy(szInfo, m_bStrict?m_oStr->GetText(ID(IDS_OPT_On), m_hInst):m_oStr->GetText(ID(IDS_OPT_Off), m_hInst));
        break;
    ...
    case OPTION_GameType:
        GetGameTypeText(m_eFCType, szInfo);
        break;
    case OPTION_Inverse:
        m_oStr->StringCopy(szInfo, m_bInverse?m_oStr->GetText(ID(IDS_OPT_On), m_hInst):m_oStr->GetText(ID(IDS_OPT_Off), m_hInst));
        break;
    case OPTION_FreeCellCount:
        GetFreeCellCountText(m_eFCCount, szInfo);
    case OPTION_NoDragging:
        break;
    }
...
void CDlgOptions::GetFreeCellCountText(EnumFreeCellCount eCount, TCHAR* szInfo)
{
    switch(eCount)
    {
    default:
    case FCCOUNT_4:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_MENU_4Free), m_hInst));
        break;
    case FCCOUNT_3:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_MENU_3Free), m_hInst));
        break;
    case FCCOUNT_2:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_MENU_2Free), m_hInst));
        break;
    }
}

void CDlgOptions::GetGameTypeText(EnumFreecellType eType, TCHAR* szInfo)
{
    switch(eType)
    {
    case FCTYPE_Freecell:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Freecell), m_hInst));
    	break;
    case FCTYPE_FourColors:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_FourColors), m_hInst));
        break;
    case FCTYPE_Kingcell:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Kingcell), m_hInst));
        break;
    case FCTYPE_Bakers:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_BakersGame), m_hInst));
    	break;
    }
}
...
void CDlgOptions::GetAutoCompleteText(EnumAutoPlay eAP, TCHAR* szInfo)
{
    switch(eAP)
    {
    case AUTOPLAY_Off:
    	...
    	break;
    case AUTOPLAY_Safe:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_Safe), m_hInst));
    	break;
    case AUTOPLAY_On:
        m_oStr->StringCopy(szInfo, m_oStr->GetText(ID(IDS_OPT_On), m_hInst));
    default:
        break;
    }
}
...
void CDlgOptions::GetItemTitle(EnumOption eOption, TCHAR* szTitle)
{
    ...
    case OPTION_PlaySounds:
     ...
     case OPTION_Skin:
        ...
        break;
    case OPTION_AutoComplete:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_AutoComplete), m_hInst));
        break;
    case OPTION_NoDragging:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_DisableDragging), m_hInst));
        break;
    case OPTION_GameType:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_GameType), m_hInst));
        break;
    case OPTION_ShowTimer:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_ShowTimer), m_hInst));
        break;
    case OPTION_Inverse:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_Invert), m_hInst));
        break;
    case OPTION_Strict:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_KingsOnly), m_hInst));
        break;
    case OPTION_FreeCellCount:
        m_oStr->StringCopy(szTitle, m_oStr->GetText(ID(IDS_OPT_AvailableCells), m_hInst));
        break;
    }
}
...
//    m_oMenu.AddCategory(_T("General"));
	...
	*eOption = OPTION_Language;
    ...
    eOption = new EnumOption;
    *eOption = OPTION_AutoComplete;
    m_oMenu.AddItem(eOption, IDMENU_Select);

  /*  eOption = new EnumOption;
    *eOption = OPTION_ShowTimer;
    m_oMenu.AddItem(eOption, IDMENU_Select);

    */

    eOption = new EnumOption;
    *eOption = OPTION_FreeCellCount;
    m_oMenu.AddItem(eOption, IDMENU_Select);

    eOption = new EnumOption;
    *eOption = OPTION_Strict;
    m_oMenu.AddItem(eOption, IDMENU_Select);

    eOption = new EnumOption;
    *eOption = OPTION_Inverse;
    m_oMenu.AddItem(eOption, IDMENU_Select);
...
//    g_gui->wndMenu.AddCategory(szTitle);
    ...
    case OPTION_Strict:
        g_gui->wndMenu.AddItem(m_oStr->GetText(ID(IDS_OPT_On2), m_hInst), IDMENU_SelectMenu, (m_bStrict?FLAG_Radio:NULL), TRUE);
        g_gui->wndMenu.AddItem(m_oStr->GetText(ID(IDS_OPT_Off2), m_hInst), IDMENU_SelectMenu, (!m_bStrict?FLAG_Radio:NULL), FALSE);
        break;
    case OPTION_ShowTimer:
        ...
        break;
    case OPTION_Inverse:
        g_gui->wndMenu.AddItem(m_oStr->GetText(ID(IDS_OPT_On2), m_hInst), IDMENU_SelectMenu, (m_bInverse?FLAG_Radio:NULL), TRUE);
        g_gui->wndMenu.AddItem(m_oStr->GetText(ID(IDS_OPT_Off2), m_hInst), IDMENU_SelectMenu, (!m_bInverse?FLAG_Radio:NULL), FALSE);
        break;
    case OPTION_PlaySounds:
    ...
    case OPTION_FreeCellCount:
        for(i=0; i < 3; i++)
        {
            EnumFreeCellCount eCount = (EnumFreeCellCount)i;
            GetFreeCellCountText(eCount, szInfo);
            g_gui->wndMenu.AddItem(szInfo, IDMENU_SelectMenu, (m_eFCCount == eCount?FLAG_Radio:NULL), eCount);
        }
        break;
    case OPTION_GameType:
        for(i=0; i < FCTYPE_Count; i++)
        {
            EnumFreecellType eType = (EnumFreecellType)i;
            GetGameTypeText(eType, szInfo);
            g_gui->wndMenu.AddItem(szInfo, IDMENU_SelectMenu, (m_eFCType == eType?FLAG_Radio:NULL), eType);
        }
        break;
    case OPTION_Skin:
        ...
        break;
    case OPTION_AutoComplete:
        for(i = 0; i < AUTOPLAY_Count; i++)
        {
            EnumAutoPlay eAuto = (EnumAutoPlay)i;
            GetAutoCompleteText(eAuto, szInfo);
            g_gui->wndMenu.AddItem(szInfo, IDMENU_SelectMenu, (m_eAutoPlay == eAuto?FLAG_Radio:NULL), eAuto);
        }
    }
...
void CDlgOptions::SetMenuSelection()
{
	...
	case OPTION_Language:
		...
		break;
    case OPTION_Strict:
        m_bStrict = (BOOL)sMenuItem->lParam;
        break;
    case OPTION_ShowTimer:
        m_bShowTimer = (BOOL)sMenuItem->lParam;
        break;
    case OPTION_GameType:
        m_eFCType = (EnumFreecellType)sMenuItem->lParam;
        break;
    case OPTION_Inverse:
        m_bInverse = (BOOL)sMenuItem->lParam;
        break;
    case OPTION_FreeCellCount:
        m_eFCCount = (EnumFreeCellCount)sMenuItem->lParam;
        break;
    case OPTION_PlaySounds:
    ...
    case OPTION_Skin:
        ...
        break;
    case OPTION_AutoComplete:
        m_eAutoPlay = (EnumAutoPlay)sMenuItem->lParam;
        break;
    }
}
